...
board_grep() {  # $1=ERE  → 번호 붙은 매칭 줄을 stdout 으로
  ...
  if [ "$rc" -gt 1 ]; then
    # ★★이 분기는 **변이가 생존한다**(2026-09-03 실측 — `if false` 로 바꿔도 락이 초록).
    #   구멍이 아니라 **도달 불가에 가까운 이중 가드**라서다: 여기 오기 전에 `require_board()` 가
    #   부재·읽기불가를 이미 걸러 내므로, 남는 것은 **읽는 도중의 I/O 오류**뿐이라 테스트로
    #   재현할 수단이 없다. 규율대로 «설명할 수 없는 생존만 진짜 구멍»이므로 **이유를 여기 적는다** —
    #   점수를 위해 억지 단언을 만들지 않는다. ★지우지는 마라: 뭉개지면 "검사기가 죽었다"가
    #   "깨끗하다"로 읽히고, 그것이 이 저장소가 `ScannerDeadError` 를 따로 만든 이유다.
