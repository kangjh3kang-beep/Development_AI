...
### Phase 2 — Lineage DAG · 모순 자동감지 ✅ 완료(2026-06-16, 7e1bad9→7cbd8dd, 7커밋)
**구현·푸시 완료.** plan=`docs/superpowers/plans/2026-06-16-phase2-lineage-contradiction.md`(0f317e7).
- 신규 `app/services/ledger/contradiction.py` — 결정론 모순탐지(status 플립+수치 델타+severity, 순수·무DB).
- 신규 `app/services/ledger/lineage.py` — lazy `_ensure` 파생그래프(record_edge[멱등 upsert·self-edge 거부]/get_parents/get_lineage[BFS]).
- 합류: design_audit(`_compare_with_prior` additive) · comprehensive(모순 표면화+lineage write-back) · feasibility(`ledger_adapters._append_with_lineage` 재사용 헬퍼, profit_rate 5%p 절대임계).
- **검증: 실 Postgres `tests/ledger` 42 passed, skipped==0 + Phase1 통합 성장루프 5 passed 회귀 무변.** 결정론 verdict/수치 절대 불변(모순=read·비교 전용), additive, 정직표기. 미배포.
- 🔧 인프라: conftest `ledger_db`에 `engine.dispose()` 추가(실행순서 무관 skipped==0 안정화).

(원 계획 — 참고) Phase 1이 깐 토대(prior read·prior_comparison·backlink·findings_brief) 위에서:
...
> 다음 세션 시작 시 권장: **Phase 2 완료(위) → Phase 3 착수.** Phase 3 그라운딩(coordinator.py:9 데드스텁·expert_panel ROSTER·W4 닫기, file:line) → writing-plans 무-플레이스홀더 TDD → executing-plans/subagent-driven. ⚠️ **WSL 명령은 PowerShell 도구로**(Bash=Git Bash라 `/home`·`/tmp` 경로변환·변수 깨짐), 실DB 테스트는 `engine.dispose()` 루프격리. (Phase 0·1·2 동일 패턴 검증됨.)
