...
- **두 표면은 같은 IA 를 접는 방식만 다르다.** 한쪽에만 처방을 적용하지 않는다.
  ...
  - 데스크톱(`sm+`): **그룹 레일** — `MENU_GROUPS` 순서로 묶고, 활성 그룹만 펼친다.
    ★**전 탭 나열 금지.**
    ★상단 높이는 「전체 탭 수」가 아니라 **「가장 큰 그룹의 크기」** 로 정해진다. 그러므로
      *"탭이 늘어도 상단이 길어지지 않는다"* 는 **그룹당 상한이 있어야만 참**이다 —
      `MENU_GROUP_MAX`(현재 9)를 넘는 그룹은 **쪼갠다**. 상한 없이 그 문장만 쓰면 거짓이다.
...
- **앱 어포던스는 두 명제를 구분한다** — 섞으면 사용자가 속는다.
  ...
  - ★**「앱 안」은 한 덩어리가 아니다 — 축마다 다른 것을 끈다.** 섞으면 자기모순이 된다.

    | 상태 | 설치 유도 | 「별도 창으로 열기」 |
    |---|---|---|
    | 설치본(`standalone`) | **끈다**(더 설치할 것이 없다) | **끈다** |
    | 별도 창(`window.name` 이 앱 창 이름) | **켠다** — 별도 창은 앱이 아니므로 설치가 여전히 개선이다 | **끈다**(무의미) |
    | 그 밖(일반 탭) | 켠다 | 켠다 |

    ★한때 이 둘을 한 판별자로 묶어 팝업 안에서 **설치를 원천 봉쇄**했다. iOS Safari 는
      `beforeinstallprompt` 가 없어 수동 안내가 **유일한 설치 경로**인데, 「별도 창으로 열기」를
      한 번 누르면 그 창에서 그 경로가 영영 닫혔다 — *"홈 화면에 추가하면…"* 이라고
      **권하면서 수단을 치우는** 상태였다.
  - 판별 **모듈**은 하나로 모은다(`lib/field-app-shell.ts`) — 한 셸 안에서 형제마다 가드를
    자기 것으로 들면 한쪽만 빠진다. **모듈은 공유하되 소비하는 축은 각자 고른다.**
