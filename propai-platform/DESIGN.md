...
### B3.3 현장앱(분양) 셸 — `components/sales-app/SiteWorkspaceClient.tsx`

현장앱은 대시보드와 **다른 제품**이다(전용 manifest·`start_url=/ko/sales/sites`·scope 분리,
PR#456). 그러므로 B3.1 공통 셸을 쓰지 않으며, 아래가 그 셸의 정본이다.

★**이 절이 없어서 실제로 결함이 났다**(2026-09-07 사용자 신고): 규정이 없으니 현장앱이
플랫폼 대시보드 관습(가로 탭바에 전 메뉴 나열)을 물려받았고, 「인지부하 해소」 처방이
**모바일 모집단에만 착지**했다.

- **정보구조는 하나다** — 탭 목록·그룹·하단 슬롯의 SSOT 는 `components/sales-app/roleConfig.ts`
  (`SALES_TABS` · `MENU_GROUPS` · `BOTTOM_NAV_KEYS`). **표면이 IA 를 재정의하지 않는다.**
- **두 표면은 같은 IA 를 접는 방식만 다르다.** 한쪽에만 처방을 적용하지 않는다.
  - 모바일(`< sm`): 하단 탭 **주 슬롯 4 + '전체' 1 = 5** 고정 + 전체메뉴 시트(그룹 그리드).
  - 데스크톱(`sm+`): **그룹 레일** — `MENU_GROUPS` 순서로 묶고, 활성 그룹만 펼친다.
    ★**전 탭 나열 금지.** 탭이 늘어도 상단이 길어지지 않아야 한다.
- **권한 게이팅은 교집합으로만**: 어느 표면이든 `visibleTabs`(내 권한) 과의 교집합만 렌더한다.
  빈 그룹은 **숨긴다**(빈 껍데기 금지).
- **앱 어포던스는 두 명제를 구분한다** — 섞으면 사용자가 속는다.
  - **설치 유도**(PWA): 여정 흐름 밖·비침투적 위치. 이미 `standalone` 이면 **숨긴다**.
    iOS Safari 는 `beforeinstallprompt` 가 **없으므로** 수동 안내(공유→홈 화면 추가)로 분기한다.
  - **창 모드 전환**(`window.open`): 이것은 **앱이 아니다.** 현대 브라우저는 팝업에서
    `location=no` 를 무시해 **주소창이 남는다**(2026-09-07 라이브 실측). 그러므로
    「앱으로 열기」라고 부르지 않는다 — 라벨은 실제로 일어나는 일을 말한다.
  - **이미 앱 안이면 어느 쪽도 노출하지 않는다**: `standalone` **또는** 팝업 내부
    (`window.name` 이 앱 창 이름). 한 셸 안에서 **형제마다 가드가 달라선 안 된다.**
- 오프라인 배너·에러 배너는 셸이 단일하게 제공한다(개별 패널이 중복 렌더하지 않는다).
- 터치 타깃 44px(§0.3), 아이콘은 Lucide 단일 세트(§0.4), 색·라운드·그림자는 Part B 토큰을 그대로 쓴다.
...
규칙:
- **유틸 이름(`shadow-md`·`shadow-2xl`)을 그대로 쓴다.** `globals.css` 의 `@theme inline` 이 Tailwind
  기본 스케일을 `--shadow-*` 토큰으로 **수렴시켜 놓았다** — 두 표기가 같은 CSS 로 컴파일된다:
  `.shadow-lg{--tw-shadow:var(--shadow-lg); box-shadow:…var(--tw-shadow)}` (라이브 CSS 실측).
  `shadow-[var(--shadow-md)]` 도 유효하나 **동치이므로 굳이 바꿀 이유가 없다.**
- **금지는 임의값이다.** `shadow-[0_2px_4px_rgba(...)]` 처럼 값을 직접 박으면 테마 전환에서 빠진다.
- 라이트는 얕고 중성(`rgba(24,33,43,.06~.15)`), 다크는 깊다(`rgba(0,0,0,.5~.95)`) —
  값의 SSOT 는 `packages/ui/src/styles/tokens.css` 의 `:root`(167~173) / `.dark`(255~261) 다.
- 그림자로 **위계를 만들되 장식하지 않는다.** 같은 층에 여러 깊이를 섞지 않는다.
- 종이 문서 뷰(등기부·보고서)는 `--shadow-lg` 사용.

> 이력 ①: v3.0 초판은 컴프만 보고 "그림자 금지"로 적었으나, 구현 SSOT 가 라이트/다크 parity 를 갖춘
> 정식 토큰 스케일을 제공하고 21개 PR 이 이를 채택한 상태였다. **문서를 코드에 맞춰 정정**(2026-07-15).
>
> 이력 ②(2026-08-16): 정정본이 이번엔 **"Tailwind 기본 스케일 ✕"** 라고 적었는데, 그것은 **Tailwind v3
> 전제**였다. v4 의 `@theme inline` 아래에서는 `shadow-lg` 가 이미 `var(--shadow-lg)` 로 컴파일되므로
> 두 표기가 **동일**하다. 이 문구 때문에 완성도 감사가 `shadow-*` 사용 **160파일·394건을 "부채"로
> 집계**했고(치환해도 산출 CSS 는 한 바이트도 안 바뀐다), 실제로 그 수치가 "디자인 정합 78%" 의
> 실점 근거가 됐다. **문서가 만든 유령 부채**다 — 다시 코드에 맞춰 정정한다.
> ★교훈: 규칙에 "✕"를 적을 때는 **그 금지가 지금 빌드에서도 참인지** 확인한다. 도구 메이저 버전이
> 바뀌면 금지의 근거가 사라질 수 있고, 문서는 그걸 모른 채 다음 사람에게 헛일을 시킨다.
