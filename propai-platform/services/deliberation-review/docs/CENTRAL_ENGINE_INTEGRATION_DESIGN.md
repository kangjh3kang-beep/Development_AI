# 중심 엔진 통합 설계 (Central Analysis Engine Integration Design) — v6

> 근거: 9-에이전트 설계 + 4-렌즈 코드대조 + **7-렌즈 9.5 시뮬레이션 4라운드**(R1 36·R2 29·R3 29·R4 23 정정, 누적 117). **R4 점수: code-fact 9.3/contract 8.5/impl 8.5/failure 9.0/invariant 9.0/security 9.0/completeness 8.5(min 8.5)** — 수렴 추이 7.0→7.5→7.5→8.5. v6=R4 정정(alembic 031 실재·테넌트 격리 write+read·divergence 산식·cross_facts 풀스키마·BimElement area=None 가드·sim/precedent verdict=None·vendored extra=forbid·결정론 제외집합 등) + **§6A 업로드/포맷/콘솔 UX 혁신(사용자 요구)** 추가.
...
- **divergence_score 산식**(0=완전일치~1=verdict 불일치): `max(verdict_mismatch, quant_rel_err)`. `verdict_mismatch`=1.0 if engine_verdict≠platform_verdict else 0(단 양측 DISCRETION_HELD/NEEDS_REVIEW면 0). `quant_rel_err`=max over k∈{area,far,bcr} of `|eng_k−plat_k|/max(|eng_k|,1e-9)`(한쪽 None은 k 제외, 둘 다 None=0). 승격 ε: 면적=quant_rel_err≤0.01 AND verdict 일치, 법규=verdict 완전일치(quant 무관). `field_diffs[]`={field, engine, platform, rel_err}.
...
- **P1**: `apps/api/app/routers/deliberation.py` = `APIRouter(prefix="/api/v1/deliberation")`; **실 가동 ASGI app `apps/api/main.py`(include_router 군집 `:591-602`)에 등록**(`apps/api/app/main.py`는 별개 공존 — 가동 app 확인). POST/GET 모두 `response_model=None`(degrade 봉투 result=null이 강타입 충돌 → 명시 dict 반환). `request_id`=헤더 `X-Request-ID` 있으면 사용, 없으면 `core/structured_logging.generate_request_id()`(`:112`)로 1회 생성·응답 헤더 echo·감사 metadata 동일값. P0 어댑터 키 주입=`apps/api`(venv)서 `python scripts/export_scoped_secrets.py --target <engine>/.env.secrets --with-db`(ALLOWLIST=ANTHROPIC/OPENAI/VWORLD/MOLIT/MOLEG, `:42-47`; API_TOKEN은 미포함→수기 별도).
