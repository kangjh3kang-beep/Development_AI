...
# 신호 키워드 → 등록 법령 ref_id(확실한 매핑만 — 불확실 카테고리[문화재/그린벨트/군사 등]는 별도 법령이라 notes로 표면화).
_SIGNAL_REFS = {
    "지구단위계획": "국토계획법§52",
    "정비구역": "도시정비법", "재정비촉진": "도시정비법", "재개발": "도시정비법",
    "재건축": "도시정비법", "도시정비": "도시정비법", "정비예정": "도시정비법",
    "고도지구": "건축법§60", "최고고도": "건축법§60",
    "경관지구": "경관법§9", "자연경관": "경관법§9", "중점경관": "경관법§9",
}
...
def upzoning_scenarios(current_zone: str | None, area: float | None,
    ...
    for step in range(0, max_steps + 1):
        ...
        add_vs_zoning = round(max_total - current_max, 1)
        rationale = Rationale(
            summary=(f"{matched}→{tz} 종상향 시 용적률상한 {far}%까지 → 최대연면적 {max_total}㎡"
                     f"(현행 용도지역 대비 +{add_vs_zoning}㎡)"),
            formula="최대연면적=목표용적률상한÷100×대지면적; 추가용량=목표최대−현행최대",
            inputs=[
                RationaleInput(name="대지면적(㎡)", value=round(area, 1)),
                RationaleInput(name="목표 용적률상한(%)", value=far, source="시행령§85 범위 내 조례 확정"),
                RationaleInput(name="현행 최대연면적(㎡)", value=round(current_max, 1)),
            ],
            legal_basis=refs("국토계획법시행령§85", "국토계획법§78"),
            caveats=[
                "용적률 상한은 이론상 최대 — 높이·일조·주차 등 타 규제로 실제 도달 불가",
                "종상향 실현은 도시계획 절차·심의·공공기여 수반(자동 아님)",
            ],
        )
        scenarios.append({
            "step": step,
            ...
            "additional_vs_current_zoning": add_vs_zoning,
...
def upzoning_signals(use_zones_all: list[str] | None) -> dict:
    ...
    # 매칭 신호 → 등록 법령 ref_id(확실한 것만; 결정론 위해 base 후 정렬). 기타 카테고리 근거는 notes로 표면화.
    matched_refs = {rid for z in zones for kw, rid in _SIGNAL_REFS.items() if kw in z}
    _base = ["국토계획법§78", "도시정비법"]  # 종상향 기본 근거(용도지역 용적률 + 정비사업)
    ref_ids = _base + sorted(matched_refs - set(_base))
    return {
        ...
        "notes": notes,
        # likelihood/신호 분류의 법령 근거(등록본 — 미상은 placeholder로 표면화, 무음 금지).
        "legal_basis": [r.model_dump() for r in refs(*ref_ids)],
    }
