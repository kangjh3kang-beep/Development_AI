...
def collect_land_card(pnu: str, stdr_year: str = "2024") -> LandCard | None:
    char = build_vworld_landchar().fetch(pnu, stdr_year)
    zones = build_vworld_landuse().land_use_zones(pnu)
    building = build_vworld_building().existing_building(pnu)
    ...
    if char is not None:
        ...
        notes.append("토지특성 결손")
    if zones:
        sources.append("vworld_landuse")
    else:
        notes.append("토지이용계획 결손")
    if building is not None:
        sources.append("vworld_building")
    else:
        notes.append("기존 건물 없음(나대지 추정)")
    ...
    if c.get("use_zone") and c.get("area"):
        ...
        rc = remaining_capacity(c["use_zone"], c["area"], existing_floor)
        if rc is not None:
        ...
        if scen is not None:
            ...
            multipath = multipath_scenarios(c["use_zone"], c["area"], sig, pnu)
            upz = {**scen, **sig}
