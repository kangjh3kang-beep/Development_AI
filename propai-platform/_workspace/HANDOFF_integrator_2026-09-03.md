...
★**상태 열을 두지 않는다.** 인계서를 쓴 시점의 CI 상태는 **몇 분 만에 뒤집힌다** —
실제로 이 문서를 쓰고 나서 `#955` 가 **린트 래칫으로 빨개졌고**, 후임 세션이
§0 지시대로 재서 잡았다. **낡음을 전제로 설계하는 것**과 **낡을 값을 아예 안 적는 것**은
한 걸음 다르다.

```bash
# ★각 PR 의 지금 상태를 먼저 재라. 아래 표의 「무엇」만 승계하고 상태는 승계하지 마라.
for N in 955 953 956 939 954 243 218; do
  gh pr view $N --json number,state,mergeStateStatus,autoMergeRequest     --jq '"#\(.number) \(.state)/\(.mergeStateStatus) AM=\(if .autoMergeRequest then "ON" else "off" end)"'
  gh pr checks $N 2>/dev/null | grep -viE 'cloudflare|workers' | head -5
done
```

| PR | 무엇 | 다음 행동 | 소유 |
|---|---|---|---|
| **`#955`** | 관리자 과금 화면이 **요율 5개를 못 바꿨다**(셋은 실제 돈) | 초록 확인 → 머지 → **158 web 배포** | 인계 시점 통합자 |
| **`#953`** | 무주인 부채 문서 | 초록이면 머지. ★**편집하지 마라** — CI 가 다시 돈다 | 〃 |
| **`#956`** | 이 인계서 | 초록이면 머지 | 〃 |
| `#939` | 계기판 유휴↔사망 | **독립 리뷰 후** | 〃 (AM 일부러 off) |
| `#954` | 사통맵 선택 필지 토글 | **손대지 않는다** | 다른 세션 |
| `#243` `#218` | 2026-07 문서 PR | 필수 체크가 완료되지 않는 상태 | 무주인 |

★**린트도 게이트다.** `#955` 가 `vitest`·`tsc` 는 초록인데 **eslint 래칫**으로 빨갰다.
래칫은 **총량이 아니라 파일 단위**로도 건다(총량 157/래칫 158 인데 **신규 파일의 경고 2건**으로 실패).
게이트를 흉내 낼 때 **일부만 흉내 내면 흉내가 아니다.**
