...
- `npm run test:run -- 'app/[locale]/(dashboard)/__tests__/dashboard-home-navigation.test.tsx' 'app/[locale]/(dashboard)/__tests__/dashboard-route-shells.test.tsx' components/layout/nav-config.test.ts lib/navigation/route-registry.test.ts`: 4 files / 20 tests 통과
...
- 전체 워크트리 `git diff --check`는 기존 사용자 변경 파일인 `services/deliberation-review/...` 공백 이슈 때문에 실패한다. 이번 웹 패치 범위는 통과했다.
