...
| **교차 테넌트 멤버십** | ★**미결정**(R3 M-4). 형제 `assign_user_to_node` 는 «같은 조직만» 을 **명문으로 강제**하는데 이 승인 경로는 테넌트를 안 본다. 사용자 요구(«모든 회원이 모든 현장») 상 **의도일 수 있으나 어디에도 안 적혀 있다** — 다음 라운드에 결정하고 **둘 중 하나를 락으로** 고정 |
