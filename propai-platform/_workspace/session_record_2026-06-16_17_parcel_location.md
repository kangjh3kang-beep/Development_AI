...
### 🔬 라이브 종단검증 (2026-06-17 완료)
- **API(공개 api.4t8t.net)**: /health 200 / **/zoning/search** 역삼동·의정부동·산12-3→PNU후보(지번/도로명 변별·다지역 변별) / **/zoning/land-share** 토지(상도159 is_aggregate=false)·집합정상(역삼757 434호 reliable=true·Σ=plat25757.2)·대단지(잠실40 455/1584 reliable=false 정직경고)·평환산(역삼1002호 16.08평·잠실101호 61.8평) 전부 정상.
- **UI(브라우저·admin 인증)**: 토지조서 페이지 인증 렌더 / 신규 **토지지번 자동완성 입력**(placeholder "입력하면 후보가 표시됩니다") present / 표 헤더 **"면적㎡(대지)"+"세대면적㎡"** 컬럼 / 행별 **"대지지분" 버튼** 렌더 / **sw v222** active.
- **헤드리스 한계(제품결함 아님)**: React onClick(모달open)·controlled-input onChange(자동완성 타이핑)·cross-origin eval-fetch는 agent-browser 헤드리스 제약으로 발화 불가 → 동일 데이터경로를 curl로 검증 + 코드리뷰 4.8/5 + tsc로 대체검증. 실사용자 환경에선 정상 동작.
