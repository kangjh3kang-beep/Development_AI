...
# §12 Phase 2 완결 — **앵커링 배포 + nonce 공개**(사용자 지시)

## ★이 절이 닫는 한계

이 PR 은 §8 에서 세 가지를 **못 한다**고 적어 뒀다. 그중 둘이 여기서 닫힌다.

| §8 의 한계 | 지금 |
|---|---|
| **1. DB 쓰기 권한자가 이긴다**(앵커 미배포·호출부 0건) | ★**배포·배선 완료** — 체인이 공약을 증언한다 |
| **3. 제3자 독립 검증 미성립**(nonce 미공개 + 멤버십 필요) | ★**무인증 검증 번들** — 최종화된 추첨에 한해 |
| 2. DB 통합 실행 미측정 | **그대로 남는다**(아래 §13) |

## 배포 — 전부 실측

    계약      DrawRegistry
    체인      Polygon Amoy(80002)
    주소      0x160F103830b5D6c7057a0FF497b60D4F3c2007B1
    가스      추정 494,079 · 한도 543,486
    배포자    0x03C39562401A94C74C8e9c103a8AC3d1D314Fa38 (잔액 0.214 POL)

★**배포 스크립트 출력은 근거가 아니다** — 체인에서 직접 확인했다:

    eth_getCode 길이           3,492자   (대조군: 코드 없는 주소 → `0x`)
    다른 제공자(drpc.org)        3,492자   일치
    owner                      배포자
    operators(배포자)           true      (대조군: 무관 주소 → false)
    ABI 호출 getDraw(0x00..)    전부 0     (계약·ABI 일치)

★**기본 RPC 는 이 환경에서 막혀 있다**(`rpc-amoy.polygon.technology` → HTTP 000).
`publicnode`·`drpc` 둘 다 chainId `0x13882` 확인.

## ★라이브 종단 — 체인이 공약 순서를 증언한다

    ① commitDraw(정직)              → anchored   (영수증 status=1)
    ② revealDraw(정직한 nonce)       → anchored
    ③ ★대조군 revealDraw(틀린 nonce) → failed     (execution reverted)

    체인에서 되읽기: commitHash 일치 · nonce 일치 · resultRoot 일치
    ★commitPrecededReveal = True     (대조군: 없는 추첨 → False)

***이 PR 이 그동안 「증명하지 못한다」고 적어 온 명제를 체인이 증언한다.***

## 의존성 — `web3` 대신 `eth-account`

    web3        새 패키지 28개(aiohttp 계열 10개 포함)
    eth-account 새 패키지 18개  ← 채택. JSON-RPC 는 **이미 있는 httpx** 로.

이 컨트랙트는 인자가 전부 `bytes32` 라 **ABI 인코더가 필요 없다**(셀렉터 + 32바이트 3개).
★그리고 직접 구현하면서 **영수증 확인을 넣었다** — 종전 구현은 전송 직후 `anchored` 를 반환했는데,
이 컨트랙트는 `revealDraw` 에서 공약을 스스로 검산하고 어긋나면 revert 한다 ⇒
***되돌려진 트랜잭션을 「체인에 올렸다」고 보고***하게 된다. **거짓 증거는 증거보다 나쁘다.**
상태를 넷으로 갈랐다: `anchored`(status=1) · `pending`(미채굴) · `failed`(revert) · `skipped`(미설정).
`pending` 은 *"모른다"* 이므로 읽을 때 `reconcile` 로 맞춘다(그대로 두면 **원장이 「모른다」로 굳는다**).

## nonce 공개 — **끝난 뒤에만**

동·호 즉석추첨은 한 명씩 뽑는다. ***중간에 nonce 를 열면 남은 대상자의 결과가 즉시 계산된다.***
⇒ `finalized_at`(전원 배정 완료 시각)이 찍힌 뒤에만 `public_commitment` 가 nonce 를 싣는다.
**그 전에는 키 자체가 없다**(빈 문자열이 아니라 부재 — 소비처 오독 방지).

## 무인증 공개 검증 — **의도한 노출, 좁힌 범위**

`GET …/draw/groups/{id}/verify` · `GET …/subscription/{id}/verify` (인증 없음)

* **최종화된 추첨만** — 진행 중이면 404
* **개인정보 0** — 이름·연락처는 **조회조차 하지 않는다**(락이 SQL 을 AST 로 검사)
* 남는 것: 해시·라운드·시각·세대 id·seed·pool — **동·호 배정 결과는 어차피 공고되는 값**
* 식별자가 UUID 라 열거 불가

★**닫으려면** 그 두 라우트에 `Depends(sales_ctx)` 를 붙이면 된다. 그때
***제3자 독립 검증은 다시 성립하지 않는다*** — 그 교환을 코드 주석에 적어 두었다.

## 잠금 — 새 락 10건

`tests/test_draw_public_verification.py`(7) + `test_draw_binding_effect_locks.py`(+2) + 앵커 락 재조준(1)

    최종화 전에도 nonce 를 싣는다            CAUGHT
    공개 엔드포인트의 최종화 게이트 제거      CAUGHT
    공개 번들이 이름·연락처를 조회           CAUGHT
    전원 완료 전에 최종화                    (두 모집단 락으로 차단)

★**부채 표식이 설계대로 작동했다**: 앵커 미배선을 알리던 `xfail(strict=True)` 가 배선하자
**XPASS 로 빨개져** 표식 제거를 강제했다.
★그리고 리팩토링이 **개인정보 락의 대상을 없앴다**(`_ABI` → `_SIGS`) — 그 락이 조용히 통과하지
않고 `AttributeError` 로 **시끄럽게** 빨개졌다. ***락의 설계가 옳았다는 증거다.***

# §13 여전히 못 한 것

1. **DB 통합 실행 미측정** — 새 컬럼 6개의 `ALTER TABLE`, `jsonb` 왕복을 실제 Postgres 로 태운 적 없다.
2. **테스트넷이다** — Amoy 는 무료 체인이고 **영속성 보장이 없다**. 실서비스는 메인넷 판단 필요.
3. **앵커 키 = 배포자 키**(현재). 분리하려면 그 주소에 `setOperator(addr, true)` 가 **먼저** 필요하다.
4. **무인증 공개 범위**는 사업 판단이다(위 교환 참조).
5. 운영자가 **결과를 보고 추첨을 미루는** 층 — 체인 시각이 이제 **드러내지만** 막지는 못한다.
