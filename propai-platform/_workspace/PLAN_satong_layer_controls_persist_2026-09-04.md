...
- ★**되돌리기 UI 미노출**: 액션은 스토어에 있으나 화면 버튼이 없다(소비처 0). 지금은 툴바에서
  ...
  갇힐 수 있었다.
- ★**하이드레이션 후 identity 교체**(미측정): 저장분이 있는 사용자는 첫 렌더가 기본값,
  재수화 후 저장값으로 바뀌며 `layerState` **identity 가 새로 생긴다**. 셸 주석이 그 귀결을
  적어 뒀다(«오버레이·POI effect 전량 파괴·재생성»). 기전은 확실하고 **체감 영향은 미측정**이다.
