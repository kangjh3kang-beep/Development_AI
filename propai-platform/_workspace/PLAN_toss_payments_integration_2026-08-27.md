...
1. ~~**환불 정책**~~ → ★**확정: 「미사용분만」**(소유자 결정 2026-08-27).
   법적 근거: 충전과 소비는 **별개 거래**이고 전자상거래법 §17②5(*"디지털콘텐츠의 제공이
   개시된 경우"*)는 **소비한 부분에만** 걸린다 → 미사용 잔액은 청약철회 대상. 부분 취소가
   법적으로도 정확한 구조이며 토스 `cancelAmount` 와 그대로 맞는다.
   **구현**: `refundable = min(주문 미환불잔액, 계정 충전잔액)` · 금액 미지정 시 미사용분 전액 ·
   명시 금액이 초과하면 **조용히 줄이지 않고 거절**(가능액 안내) · 응답에
   `unrefundable_consumed_krw` 를 실어 화면이 **왜 이만큼인지** 말하게 한다.
   ★**한계(정직)**: `coin_ledger_events` 에 **주문→소비 귀속(lot)이 없다.** 계정 전체
   충전잔액을 상한으로 쓰는 **보수적 근사**다 — 실제 미사용분보다 **적게** 환불될 수는 있어도
   **더 많이** 환불되지는 않는다. 정확한 귀속이 필요하면 lot 도입이 후속 과제다.
...
## 8. 배포 후 소유자 설정 (2026-08-27 추가)

### 8-1. ★토스 콘솔에 등록할 웹훅 URL

```
https://api.4t8t.net/api/v1/billing/payments/toss/webhook
```

**파생 근거**(추측 아님):
- 경로 = `APIRouter(prefix="/api/v1/billing")` + `@router.post("/payments/toss/webhook")`
  → ast 로 추출해 조립(`routers/billing.py`)
- 오리진 = `apps/web/lib/api-client.ts:46` `PROD_API_ORIGIN = "https://api.4t8t.net"`
- 라이브 확증: `GET https://api.4t8t.net/health` → **200** · 대조군 `GET .../billing/plans` → **200**

★**지금은 404 다** — PR #908 이 머지·배포된 **뒤에** 등록해야 한다. 배포 확인 방법:

```bash
curl -s -o /dev/null -w '%{http_code}\n' -X POST -H 'Content-Type: application/json' \
  -d '{}' https://api.4t8t.net/api/v1/billing/payments/toss/webhook
# 200 이면 배포됨(본문이 비어도 항상 200 을 돌려주는 설계) · 404 면 아직 미배포
```

**등록할 이벤트**: `PAYMENT_STATUS_CHANGED` (카드·계좌이체 등)
가상계좌를 쓸 거라면 `DEPOSIT_CALLBACK` 도 함께 — ★단 둘 다 등록하면 가상계좌 상태 변경 시
**웹훅이 두 번 온다**(토스 문서 명시). 우리 핸들러는 재조회로 확정하므로 중복은 무해하다.

★`CANCEL_STATUS_CHANGED` 는 **국내 결제에 오지 않는다**(해외 간편결제 전용 — 문서 명시).
취소 결과를 웹훅으로 기다리는 상태기계를 만들면 영원히 `pending` 에 머문다.

### 8-2. API 키 등록

**설정 › API 키 › 「결제(PG)」** 그룹에 두 키를 넣는다(CATALOG 에 있으므로 자동 노출):
- `TOSS_CLIENT_KEY` — 공개키(브라우저가 씀)
- `TOSS_SECRET_KEY` — 서버 전용

★**둘은 같은 환경의 짝**이어야 한다(둘 다 `test_` 또는 둘 다 라이브). 섞으면 토스가
`FORBIDDEN_REQUEST`/`UNAUTHORIZED_KEY` 로 거절한다 — `/admin/payments/health` 가
호출 **전에** 그것을 잡아 경고한다.

### 8-3. ★배포 전 반드시 끌 것

`BILLING_SIMULATED_PAYMENTS` 는 **결제 없이 코인을 만든다.** 기본이 `false` 이지만,
켜져 있으면 `/admin/payments/health` 가 경고하고 매출 화면의 「결제 경로별」에
`simulated` 가 **빨갛게** 뜬다. `resolve_payment_mode()` 는 토스가 설정되면 그것을 우선하지만,
레거시 `POST /billing/topup` 경로가 남아 있으므로 **끄는 것이 맞다**.
