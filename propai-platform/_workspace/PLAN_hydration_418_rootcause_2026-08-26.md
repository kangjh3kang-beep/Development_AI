...
| P9 | 새 린트 경고가 없다(#858 래칫) | `npx eslint <파일>` 전후 | **참** — 8 → 8(전부 기존 미사용 변수) |
...
- `addresses` 를 읽는 자리는 **전부 `useCallback`/`useMemo`** 이고 **마운트 이펙트 소비자가 없다**
  (파생 조회로 확인: `[addresses` dep 3곳 = 889·1023·1074 전부 콜백/메모).
...
- **클래스 ②(렌더 중 스토어 메서드 호출) 11건 / 6파일** — 라이브에서 **재현하지 않았다.
  결함이라고 부르지 않는다(미측정).** 늘지 않게 래칫만 건다.
  가장 유력한 후보는 `NextStageCta`(부분 게이트 — prop 이 오면 무가드).
...
| `lib/hydration/__tests__/render-path-store-reads.contract.test.ts` | **전수·파생형**: ①모집단(>400파일) ②탐지(양성 4) ③특이도(음성 0) ④하드 0건 ⑤클래스 ② 비성장 래칫 + **죽은 면제 실패** |
