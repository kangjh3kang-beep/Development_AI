...
### ★4-1 **소비처 0**(독립 리뷰 H2 · 2026-09-14 실측)

신설한 **기계 필드 둘**은 아직 아무도 안 읽는다. 저장소가 반복해 데인 «정의만 하고 소비처 0» 이다.

| 필드 | 소비처(정의부·본 락 제외) |
|---|---|
| `land_market_stats_zone_match` | **0파일** |
| `reference_zone_match` | **0파일** |
| `reference_note`(대조군) | **5파일** — PDF `appraisal_adapter.py:277` · web `DeskAppraisalReportClient.tsx:493` |

★즉 **사용자에게 실제로 닿는 경로는 산문 쪽**(`reference_note`)이고, 그 경로는 이 PR 에서 동작한다.
  기계 필드는 **부채**이며 `xfail(strict=True)` 로 초록 안에 드러나 있다
  (`test_debt_machine_zone_match_field_has_no_consumer_yet`) — 배선하면 **XPASS 로 실패**한다.
★web 타입(`lib/land/desk-appraisal.ts`)에도 아직 없다 — **미측정이 아니라 미배선**이다(라벨 구분).

### ★4-2 **라이브 확증의 범위**

라이브로 확증한 것은 **결함**이다(마석우리 265-1 · 일치 0.0% · 기계 필드 0건).
**수정본은 라이브에서 확증하지 않았다 — 아직 배포되지 않았다.** 이 둘을 섞어 적지 않는다.
...
| 기계 1차 | `survived=16` |
| 기계 2차(배선·분기 닫은 뒤) | `survived=8` |
| 기계 3차 | `judged=48 caught=43 **survived=5** undecided=10` |
| **독립 적대 리뷰** | ★**REQUEST CHANGES — H1·H2·H3 + M2·L1·L2** (아래) |
| **기계 4차(리뷰 봉합 후 · `--max 400`)** | `변이 74건 · **생존 8** · 판정 불가 13` |

### ★독립 리뷰가 낸 여섯 — **전부 내가 다시 재서 재현했다**(라벨 승계 없음)

| 지적 | 봉합 전 | 봉합 후 |
|---|---|---|
| **H1** 값 축 락이 부분문자열 → 실재 형제 키 `zone_type_2` 로 우회 | `SURVIVED` | **CAUGHT** |
| **H2** 첫 인자 `land_dong_stats_out → None`(재는 대상 바꿔치기) | `SURVIVED` | **CAUGHT** ×2 |
| **H3-a** 가드는 원본·비교는 `_norm` → 공백만 대상이 「쟀고 0%」로 샘 | `SURVIVED` | **CAUGHT** |
| **H3-b** `_mix_of` 센티넬 `"미상"` 을 실재 용도지역으로 셈 → 100% `match` | `SURVIVED` | **CAUGHT** |
| **M2** `(0건)` 리터럴 · 판정을 반올림된 비율에서 유추 | `SURVIVED` | **CAUGHT** |
| **L1/L2** 0% 경로만 `int` · 99.9% 를 `match` | `SURVIVED` | **CAUGHT** |
| **M3** §5 가 **존재하지 않는 락 4개**를 선언 | — | **§5 재작성 + 기계 검사** |

★**그 봉합이 또 안 잠겨 있었다**: 재판정에서 `_mix_of` 센티넬 SSOT 우회가 `SURVIVED` —
  내 락이 `land_use_mix` 를 **손으로 만들어** 소비자만 태웠기 때문이다.
  → `rows` → `_mix_of` → `zone_match` **두 층을 이어 태우는** 락 추가 후 **양방향 CAUGHT**.

### 기계 4차 생존 8건 — 전부 설명한다

| 수 | 무엇 | 판단 |
|---|---|---|
| 6 | `stats_note`·`_zone_match_clause` 의 **설명 문구**(수치도 판정어도 아닌 부분) | **의도적 미잠금** — 계약이 아니라 표현. 사유를 코드에 적었다 |
| 1 | `if not mix:` 무력화 | **등가 변이** — 빈 `mix` 는 `known` 도 비어 같은 `None`. 직접 태워 확인. 코드에 표기 |
| 1 | `if not stats:` 무력화 | ★**진짜 구멍이었다** — 무력화 시 `AttributeError`. 도달 가능(`land_dong_stats_out` 이 `None`) ⇒ 락 추가 후 **CAUGHT** |

★판정 불가 13건은 전부 **변이가 구문을 깬 것**(여러 줄 f-string·dict 리터럴). **분모가 그만큼 작다.**
★★1차 실행은 `--max` 기본값이 **14건을 버렸다**(절단은 소스 순서). *상한을 건 변이 실행은 감사가 아니다* —
  `--max 400` 으로 다시 돌린 것이 위 표다.
