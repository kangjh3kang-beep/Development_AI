...
alias_map = {
    ...
    "서울한시완화2025": "seoul_temporary_relaxation_2025",
    "서울역세권활성화조례": "seoul_transit_oriented_ordinance",
    "서울청년안심주택조례§2": "seoul_youth_housing_ordinance",
    "서울사전협상조례": "seoul_pre_negotiation_ordinance",
    "입지규제최소구역지침": "minimum_regulation_zone_guideline",
}
...
for i, line in enumerate(lines):
    if line == "}":
        # check if we are closing _ALIASES
        if i > 50 and "_ALIASES" in "".join(lines[i-30:i]):
            insert_idx = i
            break

if insert_idx != -1:
    lines.insert(insert_idx, "    # ── 심의 엔진(deliberation) 흡수 별칭 ──")
    for k, v in alias_map.items():
        lines.insert(insert_idx + 1, f'    "{k}": "{v}",')
        insert_idx += 1

with open("apps/api/app/services/legal/legal_reference_registry.py", "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
