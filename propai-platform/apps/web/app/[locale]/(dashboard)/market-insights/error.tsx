...
// 시장분석(지도 포함) 화면 전용 에러 경계.
// 이 화면 안에서만 에러를 가두므로, 사이드바·다른 메뉴 이동은 막히지 않는다.
// "다시 시도"는 이 영역만 reset 한다.
export default function MarketInsightsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    ...
    reportBoundaryError("dashboard-market-insights-error", error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 p-8">
      ...
      <h2 className="text-2xl font-black text-[var(--text-primary)]">시장분석을 불러오는 중 문제가 발생했습니다</h2>
