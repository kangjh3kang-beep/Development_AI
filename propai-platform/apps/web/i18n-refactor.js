...
let currentKoDict = {};
if (fs.existsSync(KO_JSON_PATH)) {
  currentKoDict = JSON.parse(fs.readFileSync(KO_JSON_PATH, 'utf8'));
}
...
const processFiles = (dir) => {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // Only process files that use <ModulePlaceholder> but haven't been dynamically translated yet
      if (content.includes('<ModulePlaceholder') && !content.includes('dictionary.modulePlaceholders')) {
        ...
        if (!kM[moduleKey]) continue; // Skip test folders or unknowns

        fs.writeFileSync(fullPath + '.bak', content); // Save backup

        // Fill Dictionary for this specific key
        currentKoDict.modulePlaceholders[moduleKey] = {
          eyebrow: `PROP / ${moduleKey.toUpperCase().replace('-', ' ')}`,
          title: kM[moduleKey].title,
          description: kM[moduleKey].desc,
          items: [ "✓ 데이터 실시간 연동 완료", "✓ 보안 컴플라이언스 체크", "✓ 활성 워크플로우 주입" ]
        };

        // Find the exact <ModulePlaceholder /> block and replace it
        // Regex handles multiline React component usage nicely
        const replacement = `      <ModulePlaceholder
      ...
      />`;
        ...
        content = content.replace(/<ModulePlaceholder[\s\S]*?\/>/, replacement);
...
console.log('Initiating Secure AI i18n Codemod Analysis...');
processFiles(DASHBOARD_DIR);
fs.writeFileSync(KO_JSON_PATH, JSON.stringify(currentKoDict, null, 2));
console.log(`✅ Success! Auto-translated and refactored ${modifiedCount} subpages.`);
console.log(`✅ The ko.json i18n dictionary has been seamlessly merged with the new module topologies!`);
