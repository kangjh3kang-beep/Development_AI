 ...
 *   **대표 1필지** 분석이라, 7필지 부지에서 화면이 **3,836㎡** 를 보여 주는데 같은
 *   프로젝트의 사업개요는 **164,823㎡** 를 보여 줬다 — 사용자가 어느 숫자도 믿을 수 없었다.
 *   더 나쁜 것은 그 아래 줄이다: `추정 토지가액 = 공시지가 × 대표면적` 이라 통합 부지의
 *   토지가액을 **43배 과소표시**했다(돈이 걸린 결함).
 *
 * ■ 왜 순수함수 테스트만으로는 부족한가
...
describe("LandIntelligencePanel — 면적 기준 렌더 배선", () => {
  it("★다필지(7필지) — 대표 3,836㎡ 가 아니라 통합 164,823㎡ 를 보여 주고 기준을 밝힌다", () => {
    seed(7, 7, TOTAL_AREA);
