...
describe("SatongMapShell 모바일 IA(P2) — 44px 터치 타깃 전수 불변식", () => {
  ...
  it.each([
    ["베이스맵 선택", "베이스맵 닫기"],
    // ★★2026-09-07 정정 — 종전 주석은 *"지적도는 `LAYERS_WITHOUT_POPOVER_TOGGLE` 에 들어 있어
    //   팝오버에 on/off 토글이 렌더되지 않는다"* 였다. **그 상수는 이제 존재하지 않고**(원소가
    //   둘 다 사라져 삭제됐다: `cadastre` 는 #1012 · `terrain` 은 신고② PR), 지적도는 **토글을
    //   노출한다.** 그 반대 계약이 이제 파생 락으로 고정돼 있다
    //   (`SatongMapShell.basemapSwitcher.test.tsx` — «레일의 모든 렌더러블 레이어가 토글을 노출»).
    //   ⇒ 여기서 「용도지역」을 쓰는 이유는 **토글 부재 때문이 아니라** 대표 레이어를 하나 고른 것이다.
    ["용도지역", "레이어 설정 닫기"],
  ])(
