  ...
  return (
    ...
    >
      ...
      {/* ★판별력 없는 관계는 **경보가 아니라 맥락**이다 — 상자가 이미 떠 있을 때만 밝힌다.
          ...
          ★★**문구를 상태별로 가른다**(2차 적대 리뷰 MAJOR-1). 첫 판은 *"…은 **위 수에 포함돼
          있으나** 결과를 가르지 못합니다"* 를 **조건 없이** 모든 상태에 붙였는데, 「위 수」가
          상태마다 **다른 것**을 가리킨다. 내가 직접 렌더해 관측한 출력:

            [violations] "전제 불일치 검출 · **1건**" … "판별력이 없는 검사 **1건**(…)은
                         **위 수에 포함돼 있으나** 결과를 가르지 못합니다"
                         → 읽는 사람은 «검출된 1건 = 판별력 없는 그 1건» 으로 읽고
                           **진짜 위반을 기각한다.** 그 위반은 `count_conservation_parcels`
                           (필지 붕괴)인데 문장이 그것을 다른 검사라고 부른다.
            [vacuous]    "한 건도 실행하지 못했습니다" … "위 수에 포함" → **0에 1이 포함될 수 없다.**

          ★**가장 비싼 순간(진짜 위반이 잡힌 순간)에만 발화하는 거짓**이었다 —
            이 기능이 존재하는 이유 그 자체의 반전이다.
          ★그리고 락 22건이 전부 초록이었다: 「위반」 픽스처가 이 문장을 **실제로 렌더하는데**
            단언이 `title`·`detail`·`state` 뿐이라 **아무도 안 봤다**(초록 안에서 거짓이 출력됐다). */}
      {vacuous > 0 && state !== "vacuous" && (
        <p className="mt-1.5 text-[10px] text-[var(--text-hint)]">
          {state === "partial"
            /* 「위 수」 = 실행 수. 판별력 없는 관계도 **실행은 됐으므로** 포함이 참이다. */
            ? `참고 — 이 경로에서 판별력이 없는 검사 ${vacuous}건(${vacuousKeys.join(", ")})은 위 실행 수에 포함돼 있으나 결과를 가르지 못합니다.`
            /* 위반 상태 — 「위 수」는 **위반 개수**다. 포함을 주장하지 않고 **무관함을 명시**한다. */
            : `참고 — 이 경로에서 판별력이 없는 검사가 ${vacuous}건 있습니다(${vacuousKeys.join(", ")}) — 위 검출 건수와는 무관합니다.`}
        </p>
      )}
