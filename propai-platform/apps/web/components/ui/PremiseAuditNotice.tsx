 ...
 *
 * ## ★`structurally_vacuous` — 판정력의 분모에서 뺀다
 *
 * `#963` 이 시뮬레이터 경로에서 `path_invariance_zone` 을 `structurally_vacuous` 로 표시한다.
 * 그 경로는 집계와 시나리오가 **같은 `primary_zone`** 에서 나와 **자기 자신과 비교**하므로
 * 그 관계는 원리적으로 아무것도 못 가른다. **그런데 `checked` 는 그것을 여전히 센다** —
 * 그대로 쓰면 화면이 **판정력을 과대표시**한다.
 *
 * ★이 저장소가 이미 값을 치른 형태다: *"「100%」는 지표를 재정의해서 달성하는 것이 가장 쉽다"*.
 *   그래서 **둘 다 싣는다** — 실효 판정력과 원래 `checked` 를 함께 말한다(한 수로 뭉개면 오독된다).
 *
 * ## 무날조
 *
...
export function premiseAuditState(a: PremiseAudit | null | undefined): PremiseAuditState {
  ...
  // ★**해석할 수 없는 형태에는 아무 주장도 하지 않는다.** 백엔드는 성공·실패 **양쪽 경로 모두**
  //   `checked` 를 싣는다(실패 경로도 `checked: 0`). 그것이 없으면 이 페이로드는 감사 결과가
  //   아니거나 계약이 깨진 것이고, 그때 「감사가 한 건도 판정되지 않았습니다」라고 **외치면
  //   그것 자체가 위양성**이다 — 가드가 정상 화면에 경고를 붙이면 그것도 결함이다.
  //   ★내 첫 판이 `[]` 를 「공허」로 판정했고 이 락이 그것을 잡았다.
  if (!Number.isFinite(a.checked)) return "clean";
  const violations = Array.isArray(a.violations) ? a.violations.filter(Boolean) : [];
  if (violations.length > 0) return "violations";
  const { effective, registered } = premiseAuditPower(a);
  if (effective === 0) return "vacuous";
  if (registered != null && effective < registered) return "partial";
  return "clean";
}

/**
 * 판정력 — ★`structurally_vacuous` 를 **뺀다.**
 *
 * `checked` 를 그대로 쓰면 «원리적으로 아무것도 못 가르는 관계»가 판정 수에 섞여
 * **판정력이 과대표시**된다. 원래 `checked` 도 함께 돌려줘 호출부가 **둘 다** 말할 수 있게 한다.
 */
export function premiseAuditPower(a: PremiseAudit | null | undefined): {
  ...
  const checked = Number.isFinite(a?.checked) ? Number(a?.checked) : 0;
  const vacuousList = Array.isArray(a?.structurally_vacuous) ? a!.structurally_vacuous!.filter(Boolean) : [];
  const vacuous = vacuousList.length;
  const registered = Number.isFinite(a?.registered) ? Number(a?.registered) : null;
  // ★음수로 내려가지 않게 하되, **0 으로 뭉개지 않는다** — 두 수의 관계가 신호다.
  return { effective: Math.max(0, checked - vacuous), checked, vacuous, registered };
}
...
export function PremiseAuditNotice({
  ...
  const { effective, checked, vacuous, registered } = premiseAuditPower(audit);
  ...
  const heading =
    ...
    : state === "vacuous" ? "전제 감사가 한 건도 판정되지 않았습니다"
    : `전제 감사 부분 판정 · ${effective}/${registered ?? "?"}`;
  ...
  return (
    ...
    >
      ...
      {state === "vacuous" && (
        ...
        <p className="text-[10px] leading-relaxed text-[var(--text-secondary)]">
          아래 판정을 교차검증할 전제 관계가 하나도 성립하지 않았습니다 — 「이상 없음」이 아니라
          「확인하지 못함」입니다.
          {vacuous > 0 ? ` (구조상 판정 불가 ${vacuous}건 제외 · 원 검사 ${checked}건)` : ""}
        </p>
      )}
      ...
      {state === "partial" && (
        ...
        <p className="text-[10px] leading-relaxed text-[var(--text-secondary)]">
          등록된 {registered}개 전제 관계 중 {effective}개만 판정했습니다 — 나머지는 입력이 부족해
          건너뛰었습니다. 위반 0이 전체를 보증하지 않습니다.
          {vacuous > 0 ? ` (구조상 판정 불가 ${vacuous}건 제외 · 원 검사 ${checked}건)` : ""}
        </p>
      )}
