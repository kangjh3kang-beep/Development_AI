...
describe("콜백 화면 문구 위생", () => {
  ...
  it.each(discoverCallbackClients())(
    ...
    (file) => {
      ...
      // ★목록이 아니라 **정본에서 파생**하고, 그 위에 **양방향 집합 비교**를 건다.
      //   2026-09-06 실측: 「for (const loc of locales)」 로 파생만 시켰더니
      //   그것을 손수 목록 ["ko","en"] 으로 되돌리는 변이가 **SURVIVED** 였다 —
      //   각 로케일이 «있는가»만 보면 **목록을 줄여도 아무도 실패하지 않는다.**
      //   그래서 컴포넌트가 실제로 가진 키 집합을 뽑아 정본과 **양방향으로** 맞춘다:
      //   빠진 로케일(누락)도, 정본에 없는 로케일(유령)도 둘 다 빨개진다.
      expect(locales.length, "로케일 모집단이 비었다 — 조회기 사망").toBeGreaterThanOrEqual(3);
      const found = [...block.matchAll(/(?:^|[\s{,])"?([a-z]{2}(?:-[A-Z]{2})?)"?\s*:\s*\{/g)].map(
        (m) => m[1],
      );
      expect(new Set(found), `${file}: 로케일 키 집합이 정본과 다르다`).toEqual(new Set(locales));
    },
  );
