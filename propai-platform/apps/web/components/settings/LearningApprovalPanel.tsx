 ...
 *
 * 【변이 감사 분류 — 재검증 중(2026-08-19)】
 * 앞서 이 자리에 남은 생존을 전부 표시층으로 적었는데 그 분류는 **틀렸다**. 적대리뷰가
 * fetch URL 의 경로 끝에 글자를 붙이는 변이(/candidates → /candidatesX)를 넣었더니
 * 살아남았다 — 문자열 변이지만 표시층이 아니라 프로덕션 404 를 내는 배선 구멍이었다.
 * 원인은 모든 경로 단언이 부분문자열이었던 것이고 정확 일치로 고쳤다.
 * 남은 생존의 재분류(표시문구 / 배선문자열 / 런타임소멸)는 이 커밋 뒤 도구 재실행으로
 * 채운다 — 그 전까지 "남은 건 다 표시층"으로 읽지 마라.
