 ...
 *   ② 배선 문자열(URL·경로·쿼리키·요청본문키·**상태값**) …… 3건 → **전부 진짜 구멍이었다**
 *        (잠근 뒤 도구 재실행: 136변이·생존 60 · **② 0건**)
 *        · STATUS_TABS 의 key "candidate"/"rejected" — 표시 문구가 아니라 서버로 가는 값이다
 *          (바꾸면 그 탭을 누를 때 400). 탭을 DOM 에서 파생해 요청 status 집합을 잠갔다.
 *        · statusClasses 의 case "rejected" — 거부됨 탭 렌더 테스트로 잠갔다.
 *   ③ 런타임에 사라지는 것(use client 지시자·TS 유니온 타입·React key) …… 4건
 *        jsdom 테스트로는 **원리적으로** 못 잡는다. type-check 가 담당한다.
 *   ① 표시 문구·클래스명 …… 58건. 바꿔도 동작이 같다.
 * ★교훈: 문자열 변이를 한 덩어리로 분류하면 그 안에 배선 구멍이 숨는다. 새 로직을 넣으면
 *   도구를 다시 돌리고 **종류별로** 가려라("남은 건 다 표시층"이라고 읽지 마라).
