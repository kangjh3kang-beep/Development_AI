...
const stats = [
  {
    // ★2026-09-07(사용자 신고②) — **12 → 11**. 「지형도·항공뷰」 레이어를 레일에서 없앴다.
    //   그것은 레이어가 아니라 **베이스맵 스위처의 복제본**이었다(컨트롤 id 4/5 동일 ·
    //   같은 핸들러 · 같은 저장 위치 · 같은 `resolveVWorldBaseLayer()`). 라벨만 둘이었다.
    //   ★그래서 **기능이 줄어든 것이 아니라 중복 표기가 줄었다** — 베이스맵 전환은 그대로다.
    //   위 주석대로 `test_ui_capability_claims_are_backed_by_code.py` 가 이 수를 배열에서
    //   파생해 잠그고 있었고, **그 락이 이 거짓 주장을 CI 에서 잡았다**(assert [12,9,3] == [11,9,3]).
    value: "11",
    unit: "종",
    label: "지도 데이터 레이어",
