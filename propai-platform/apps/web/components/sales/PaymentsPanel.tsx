...
export default function PaymentsPanel({ siteCode }: { siteCode: string }) {
  ...
  const [pay, setPay] = useState({ va_number: "", amount: "" });
  ...
  const ingest = async () => {
    ...
    const r = await api.post<{ matched: boolean }>("/payments/webhook", { va_number: pay.va_number, amount: Number(pay.amount) });
    setMsg(r.matched ? "입금 대사 완료(회차 충당)" : "미매칭 — 수동 대사 큐로 이동"); setPay({ va_number: "", amount: "" }); load();
  };
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-soft)] p-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input value={pay.amount} onChange={(e) => setPay({ ...pay, amount: e.target.value })} type="number" placeholder="입금액(원)" className={IN} />
