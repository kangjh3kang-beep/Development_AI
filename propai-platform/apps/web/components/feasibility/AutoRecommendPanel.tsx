...
export function AutoRecommendPanel({ onClose, isModal = false, embedded = false }: AutoRecommendPanelProps) {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      //   best-effort 병렬 호출해 종상향(far_basis='종상향') 후보를 surface(배너→실랭킹). 실패해도 본 추천 불변.
      try {
        const addrs = [address.trim(), ...parcels].filter(Boolean);
        const opt = await apiClient.post<OptimalRecommendResponse>("/development-methods/optimal-recommend", {
          body: { addresses: addrs, parcel_subset_policy: "전체" },
          useMock: false,
        });
        const upRanked = (opt?.ranked ?? [])
          .filter((c) => c.far_basis === "종상향" && (c.net_profit != null || c.profit_rate_pct != null))
          .slice(0, 4);
        setUpzoningRanked(upRanked);
      } catch {
        ...
        <div className="rounded-[2rem] border border-amber-500/30 bg-amber-500/5 p-6">
          ...
          {upzoningRanked.length > 0 && (
            <div className="mt-3 border-t border-amber-500/20 pt-3">
              <p className="mb-2 text-xs font-bold text-amber-400">종상향 시 추천 사업방식(수익순·잠재)</p>
              <div className="space-y-1.5">
                {upzoningRanked.map((c, i) => (
                  <div key={i} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[var(--line)] bg-[var(--surface-strong)] px-3 py-2">
                    ...
                    <span className="flex items-center gap-2 text-[10px]">
                      ...
                      {c.composite != null && <span className="text-[var(--text-tertiary)]">종합 {c.composite}</span>}
                    </span>
                  </div>
                ))}
              </div>
