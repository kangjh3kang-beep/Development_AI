    ...
    >
      ...
      <div className="px-4 py-3 border-t border-[var(--line)]">
        <p className="text-xs font-semibold text-[var(--text-secondary)] mb-2">
          연동 검산 — 이 표는 산출 엔진을 <b>참조만</b> 합니다. 아래가 모두 OK 여야 유효합니다.
        </p>
          ...
          {ledger.checks.map((c) => (
            <li key={c.key} className="flex flex-wrap items-center gap-2 text-xs">
              <span
                ...
                className={`px-1.5 py-0.5 rounded font-bold ${VERDICT_STYLE[c.verdict].cls}`}
              >
        ...
        {hasError && (
          <p className="mt-2 text-xs text-[var(--status-danger)]">
            ★ 합계가 어긋납니다. 이 표의 숫자를 의사결정에 쓰기 전에 원인을 확인하십시오.
          </p>
        )}
      </div>
