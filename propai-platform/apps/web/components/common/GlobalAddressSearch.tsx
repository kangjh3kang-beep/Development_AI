...
export interface AddressEntry {
  ...
  // ── 토지조서 동의 3종(O/X) — **사용자 파일에 그 칸이 있으면** 읽는 값이다 ──
  // ★2026-09-12 정정: 종전 주석은 *"양식에서 직접 채우는 값"* 이라고 적었는데, 표준 양식이
  //   **주소·지번 두 칸으로 축소**되면서 그 문장이 거짓이 됐다. 지금 이 값의 출처는
  //   **사용자가 이미 쓰던 자기 양식**이다(파서의 헤더 별칭 `_H_CONSENT_*` 가 그대로 인식한다 —
  //   `test_parcel_template_minimal_columns.py` 가 그 동작을 태워 확인한다).
  // ★여기 선언이 없으면 업로드 응답에 실려 온 값이 타입 경계에서 조용히 버려진다
...
export function GlobalAddressSearch({
  ...
  }, [single, addresses, updateSiteAnalysis, onChange, writeToContext, triggerComprehensiveAnalysis, enrichParcels]);
  ...
  const handleRemove = useCallback((index: number) => {
    ...
    onChange?.(newAddresses);
  }, [addresses, updateSiteAnalysis, onChange, writeToContext]);
  ...
  }, [directQuery, directBusy, handleAddressSelect, single]);
