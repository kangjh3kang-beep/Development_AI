        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="rounded-xl border border-[var(--accent-strong)]/30 bg-[var(--accent-strong)]/5 p-4">
            <div className="flex items-center justify-between">
              <p className="text-xs font-black text-[var(--accent-strong)]">🤝 다관점 통합 결론</p>
              <div className="flex items-center gap-2">
                {conf != null && (
            ...
            {result.experts.map((e, i) => (
              <div key={i} className="rounded-xl border border-[var(--line)] bg-[var(--surface-strong)] p-3.5">
                ...
                {(e.concerns?.length ?? 0) > 0 && (
                  <p className="mt-1.5 text-[11px] text-amber-500">⚠ {e.concerns!.join(" / ")}</p>
                )}
              </div>
            ))}
          </div>
          ...
          {result.verification && (
            <div className="grid gap-3 md:grid-cols-3">
              <VBlock title="🔍 검증 리스크" items={result.verification.risks} tone="rose" />
              <VBlock title="↔ 반론·맹점" items={result.verification.counterpoints} tone="amber" />
              <VBlock title="📋 추가 확인 필요" items={result.verification.data_gaps} tone="sky" />
            </div>
          )}
        </div>
      )}
...
function VBlock({ title, items, tone }: { title: string; items?: string[]; tone: string }) {
