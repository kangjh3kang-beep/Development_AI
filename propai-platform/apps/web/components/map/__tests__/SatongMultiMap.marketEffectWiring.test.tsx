...
describe("★★호출부 배선 — effect 가 `buildMarketMarker` 에 **무엇을 넘기는가**", () => {
  ...
  it("★★MAJOR-1c — 전월세 모드가 **라벨 표기까지 전월세**다(`kind`)", async () => {
    ...
    // ★팝업을 클릭으로 열려 했으나 jsdom 에서 Leaflet path 클릭이 팝업을 열지 못한다(실측).
    //   그래서 **열지 않고도 보이는** 라벨을 매체로 쓴다. 팝업 본문 축은 `buildMarketMarker`
    //   단위 하네스(`SatongMultiMap.markerWiring.test.ts`)가 이미 태운다.
    await 그리기({
