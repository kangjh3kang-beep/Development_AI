 ...
 *
 * ★"확대하세요"로 끝내지 않고 **이 배율에서 무엇이 되는지**를 말한다(2026-08-13 실측).
 *   같은 BBOX 를 줌별로 재 보면 저배율에서도 용도지역은 실제로 그려진다:
 *     강남 z15  — 지적 1,784B(빈 타일) · 용도지역 6,674B
 *     호미곶 z15 — 지적 1,784B(빈 타일) · 용도지역 4,892B
 *     호미곶 z18 — 지적 5,360B(렌더)   · 용도지역 3,614B
 *   즉 저배율은 "아무것도 없는 상태"가 아니다. 막다른 안내는 사용자를 더 헤매게 한다.
 *   ★lt_c_upisuq161 은 강남에서만 렌더되고 호미곶은 빈 타일이라(8,319B vs 1,784B)
 *     지역 편차가 있어 안내에 넣지 않았다 — 늘 참인 것만 말한다.
 */
export const CADASTRE_ZOOM_HINT =
  `지적도는 z${CADASTRE_MIN_ZOOM} 이상에서 제공됩니다 — 확대하면 지번·경계가 표시됩니다`
  + " (이 배율에서는 용도지역 레이어가 표시됩니다)";
