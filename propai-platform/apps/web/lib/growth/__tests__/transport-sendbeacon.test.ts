...
describe("★전송 — 프로덕션 분기(sendBeacon)를 실제로 태운다", () => {
  ...
  it("★B `sendBeacon` 이 **false** 를 주면 `fetch keepalive` 로 폴백한다(예산 초과 경로)", async () => {
    ...
    //
    // ★**이 수정이 산 것을 과장하지 않는다.** 같은 변이를 **3분할**로 재니:
    //     ① 이 단언 단독            → **CAUGHT**
    //     ② 형제 파일 단독          → **CAUGHT**(10 failed) ← **저장소 전체로는 이미 덮여 있었다**
    //     ③ 이 파일에서 이 단언 제외 → **SURVIVED**
    //   즉 이 수정이 닫은 것은 *"저장소의 구멍"* 이 아니라 **「이 락이 자기가 선언한 것을
    //   실제로 단언한다」** 이다. 초판 B 는 *"폴백한다"* 고 **제목에 선언**해 놓고 개수만 보며
    //   **형제에 얹혀 있었다.** 그것으로 충분히 고칠 값어치가 있지만, **저장소 커버리지를
    //   새로 만들었다고 말하면 거짓**이다(이 PR 초판이 정확히 그 과장으로 근거가 반증됐다).
