 ...
 *     인라인 `transform`·수작업 CSS는 **전부 사각지대**다. 실측된 사각 예:
 *     `app/globals.css` 의 `.glass`(backdrop-filter, 사용 27회) · `.leaflet-container`
 *     (isolation:isolate) · `.cc-panel`(overflow:hidden, 사용 33파일) · `.sa-di-block`(overflow:hidden).
 *     jsdom 은 CSS 를 로드하지 않으므로 `getComputedStyle` 로도 구제되지 않는다 —
 *     이 층까지 보려면 **실제 브라우저에서 페인트 순서(`elementFromPoint`)로** 재야 한다.
 *   · `opacity-[var(--x)]` 처럼 값이 런타임 변수면 정적으로 결정할 수 없다 — 안전한 쪽(위반)으로 센다.
...
export const STACKING_UTIL = util(
  [
    ...
    String.raw`(?:scale|rotate|translate|skew)(?:-[xyz])?-(?:\[[^\]]*\]|[\d./]+|px|full)`,
    String.raw`perspective(?:-origin)?-[a-z0-9[\]./-]+`,
    // filter 계열 — `filter: <fn>`은 none이 아니면 전부 층 상자
    String.raw`(?:backdrop-)?blur(?:-[a-z0-9[\]./]+)?`,
    String.raw`(?:backdrop-)?(?:brightness|contrast|saturate|hue-rotate|drop-shadow|opacity)-[a-z0-9[\]%./-]+`,
    String.raw`(?:backdrop-)?(?:grayscale|invert|sepia)(?:-[a-z0-9[\]./]+)?`,
...
/** `overflow-hidden|clip|auto|scroll`(x/y 포함) — 팝오버를 **잘라내는** 조상. */
export const CLIPPING_UTIL = util(String.raw`overflow(?:-[xy])?-(?:hidden|clip|auto|scroll)`);
...
export function scanAncestorTraps(
  ...
  while (node && node !== node.ownerDocument?.body) {
    ...
    if (kinds.includes("clipping") && clipsDescendants(cls)) {
      traps.push({ element: node, depth, className: cls.slice(0, 90), kind: "clipping" });
    }
...
export function describeTraps(traps: LayerTrap[]): string {
  return traps
    .map((t) => `  · [조상 ${t.depth}] ${t.kind === "stacking" ? "층 상자 생성" : "잘라냄"}: ${t.className}`)
    .join("\n");
}
