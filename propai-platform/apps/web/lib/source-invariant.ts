 ...
 *
 * ★변이검증 — **방법을 정직하게 적는다**(독립 검증 지적):
 *   저장소 도구 `scripts/mutate_changed.py` 는 **`.py` 전용**이라 이 PR(.ts/.tsx)에는
 *   **한 건도 돌지 않는다**. 그래서 같은 규칙(diff 추가줄에서 기계 추출)을 TS 에 적용하는
 *   **임시 어댑터**로 돌렸다. 즉 "변이를 내가 고르지 않는다"는 그 실행분에는 참이지만,
 *   커밋에 나열한 V1~V4 같은 개별 변이는 **손으로 고른 것**이다 — 둘을 구분해서 읽어라.
 *   ★재현 가능하게 하려면 본체에 TS 지원을 넣어야 한다(후속 티켓).
 * ★설명되는 생존 유형(반복 확인된 것):
 *   · **타입 필드 삭제** — vitest 는 타입을 지우고 돌리므로 생존이 당연하다. 실측: `minFiles` 를
 *     지우면 `tsc --noEmit` 이 TS2353 1건 + TS2339 2건을 낸다(= type-check 게이트가 지는 몫).
 *   · **오류 메시지 문구** — 던지는 조건은 픽스처로 잠겨 있다(backdrop.test.ts 의 throw 3종).
 *     문구 자체는 사용자 산출물이 아니라 개발자 진단이라 계약이 아니다.
 *   · `const classes = raw.replace(/\s+/g," ").trim()` — `HAS_CLASS`·`Z_UTIL` 이 `(?:^|\s)`·
 *     `(?=\s|$)` 로 개행을 흡수하므로 검출 결과가 같은 **등가 변이**다(보고 문자열 가독성만 달라진다).
 *   ★"설명 못 하는 생존 0" 은 **그 실행 시점의 코드에 대한 주장**이다. 코드를 고친 뒤에는
 *     다시 돌려 분류하기 전까지 그 문장을 그대로 재사용하지 마라(한 번 그렇게 해서 틀렸다).
 * ──────────────────────────────────────────────────────────────────────────── */
