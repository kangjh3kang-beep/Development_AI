...
const WIPE_EXEMPT: Record<string, string> = {
  ...
  // ★부채 — 아래 둘은 **유료·비가역 산출물**이고 **브라우저 로컬에만** 있다.
  //   지우면 사용자가 이미 낸 돈이 사라지고(렌더 3,000원/건 · 등기 권리분석 1,200원/필지),
  //   등기 쪽은 서버 복원을 **의도적으로 거부**했다(테넌트 공유 캐시를 무과금으로 열면
  //   임의 주소로 소유자 정보를 수확할 수 있게 된다 — `useRegistryAnalysisStore` 독스트링).
  //   그렇다고 안 지우면 같은 브라우저의 **다음 계정이 이전 계정의 유료 산출물을 본다**.
  //   ★올바른 처방은 와이프가 아니라 **계정별 키 격리**다(선례: `propai_pipeline_history__`
  //     — 키가 갈려 있어 격리와 보존을 동시에 만족한다). 키를 바꾸면 기존 저장분을 옮기는
  //     마이그레이션이 함께 필요하므로(안 하면 그 순간 유료 산출물이 고아가 된다) 별건으로 둔다.
  //   여기 적어 두는 이유: 미분류 래칫에 밀어 넣으면 "아직 안 본 것"으로 읽히는데,
  //   이건 **보고 나서 미룬 것**이다. 둘은 다르게 읽혀야 한다.
  "propai-paid-renders":
    "★부채 · 유료 AI 렌더 결과(건당 3,000원)의 브라우저 로컬 유일본 — 지우면 산 것이 사라진다. 격리는 와이프가 아니라 계정별 키로 풀어야 한다(마이그레이션 동반 · 별건)",
  "propai-registry-analysis":
    "★부채 · 등기 권리분석 결과(필지당 1,200원)의 브라우저 로컬 유일본 — 서버 복원은 소유자 정보 수확 노출면 때문에 의도적으로 거부됐다. 격리는 계정별 키로 풀어야 한다(마이그레이션 동반 · 별건)",
};
...
const UNTRIAGED_KEYS = [
  "propai-development-plan", "propai-field-app", "propai-nav-expanded",
