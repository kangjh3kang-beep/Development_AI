...
const WIPE_EXEMPT: Record<string, string> = {
  ...
  // ★2026-09-04 신규 — **처음부터 계정별**이다(레거시 공유키가 없다). 이 키 이름 자체로는
  //   아무것도 저장되지 않고 실제 저장은 `propai-satong-map-prefs__<uid>` 로 간다.
  //   ①**키는 갈려 있다**: 실제 저장은 `__<uid>` 로 가므로 다른 계정의 **키를 읽지는 않는다.**
  //   ★2026-09-04 정정: 초판에 *"다음 계정이 이전 계정의 선호를 **못 본다**"* 고 적었는데
  //     **거짓이었다**(적대 리뷰가 실행으로 반증). 소프트 계정 전환에서는 메모리 상태가
  //     그대로 남아 다음 계정이 **이전 값을 본다** — 키 분리는 참이지만 거기서 끌어낸
  //     결론이 틀렸다. 그래서 `syncAccountScopedStores()` 에 재수화를 등록했다.
  //   ②**지우면 안 되는 이유**: 이건 «사용자가 스스로 끈 것»이다. 로그아웃마다 지우면
  //     206필지 작업에서 매번 다시 꺼야 한다 — 그 반복이 이 스토어를 만든 이유 그 자체다.
  //   ★이 락이 **새 키를 정확히 지목해서** 이 판단을 하게 했다(파생형의 값).
  "propai-satong-map-prefs":
    "사통맵 레이어 컨트롤 선호 — 처음부터 계정별(`__<uid>`)이라 교차계정 누출이 구조적으로 불가능하고, 사용자가 스스로 끈 상태를 로그아웃마다 되돌리면 매번 다시 꺼야 한다(이 스토어의 존재 이유)",
};
