...
describe("④ 공사비 배선 — 단위·폴백·정본", () => {
  ...
  it("★평당↔㎡ 배수가 프로덕션 코드에서 이 모듈 밖에 있으면 실패한다(SSOT)", () => {
    ...
    // ★2026-09-07 11 → 10. `PricingBandPanel` 의 `PYEONG_SQM = 3.305785` 평면 상수를
//   걷어내고 `area-notation` 정본을 import 하도록 바꿨다(전용률 SSOT 작업의 부수 효과).
//   ★래칫은 **양방향**이다 — 줄었으면 내려서 고정한다. 안 내리면 다음 사람이
//   상수를 하나 되살려도 초록이다.
const RATCHET_MAX = 10;   // 2026-09-06 측정치. ★줄었으면 이 수를 내려라.
    expect(hits.length, (
