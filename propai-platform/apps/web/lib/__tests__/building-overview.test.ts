...
/**
 * ★★MAJOR-3(2026-09-12 적대 리뷰) — 어댑터의 **런타임 소비처가 0건**이다.
 *
 * 계획서 §5 는 *"어댑터가 실제로 쓰인다 | 모듈 객체 동일성(`is`)으로 소비처 배선 확인"* 을
 * 잠금으로 선언했으나 **그 락은 존재하지 않는다** — 소비처가 0이라 **원리적으로 작성 불가**다.
 * 측정(2026-09-12):
 *     grep -rn "building-overview-adapters" --include=*.ts --include=*.tsx
 *       → 정의 1 · 이 테스트 파일 1 · **런타임 임포트 0**
 *
 * ★그러므로 사용자 요청의 *«다른 보고서에 글로벌 적용»* 은 현재 **0건 달성**이다.
 *   계획서 §3 은 *"94개 중 몇 곳이 전환되는지 미측정"* 이라 적었는데, 이제 **쟀고 0** 이다.
 *   ***「미측정」과 「0」은 다르다*** — 전자는 모름이고 후자는 사실이다. 바꿔 적는다.
 *
 * ★이 부채를 커밋 메시지가 아니라 **초록 안에** 둔다(§B-13). 아래 todo 가 닫히는 날은
 *   소비처를 한 곳이라도 실제로 옮긴 날이다 — 그때 비로소 「동일성 락」을 쓸 수 있다.
 */
describe("★부채 — 어댑터 배선", () => {
  it.todo(
    "★어댑터를 실제로 임포트하는 **런타임 소비처**가 있고, 모듈 객체 동일성으로 배선을 잠근다 " +
      "— 현재 소비처 0건이라 작성 불가. 첫 소비처(kr-construction-cost 호출부)를 옮긴 뒤 상환한다.",
  );
});
