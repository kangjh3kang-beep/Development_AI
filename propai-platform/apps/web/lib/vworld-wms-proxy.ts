 ...
 *
 *   ★★2026-08-17 정정 — 종전 이 자리의 **"VWorld 키는 두 계약으로 나뉜다"는 서술은 실측으로
 *   거짓이었다.** `.env` 의 `VWORLD_API_KEY` 와 `NEXT_PUBLIC_VWORLD_API_KEY` 가 **같은 값**이다
 *   (sha256 대조 일치·둘 다 36자). `.env.example` 도 두 줄에 같은 플레이스홀더를 쓰고,
 *   `Dockerfile.web` 이 `NEXT_PUBLIC_*` 를 빌드 ENV 로 굽는다 — 즉 "서버 전용"이라 부른 그 값이
 *   **이미 브라우저 번들에 들어 있다.** 이 프록시의 키 은닉 이득은 현 설정에서 ≈0 이며,
 *   그것은 코드 결함이 아니라 **키 발급·설정 결함**이다.
 *   → 복구 조건: VWorld 콘솔에서 키를 2개로 분리 발급(공개키=Referer/도메인 제한 · 서버키=미노출)
 *     하고 **노출 이력이 있는 현 서버키를 폐기·재발급**해야 위 문장이 비로소 참이 된다.
 *   ※이 주석을 "이미 분리돼 있다"로 되돌리지 마라 — **값을 다시 재고 나서** 고쳐라.
 *
