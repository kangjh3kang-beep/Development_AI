...
/* ★`salesApi.clearSiteToken` 이 이 모듈을 **런타임으로** 임포트하게 되면서(2026-09-09)
   경계가 넓어졌다 — 종전엔 `import type` 한 방향뿐이었다. 현 소비처는 전부 client 이고
   `create()` 는 window 를 안 만지므로 실해는 없지만, 서버 컴포넌트가 `salesApi` 를
   임포트하면 조용히 끌려온다. 지시자를 달아 그 경우가 **빌드에서 드러나게** 한다
   (2026-09-12 · 리뷰 MINOR 6). */
/**
