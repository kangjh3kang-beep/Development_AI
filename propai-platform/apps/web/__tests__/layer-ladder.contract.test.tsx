...
describe("앱 전역 층위 사다리", () => {
  ...
  it("★지도 공존 화면의 모달을 **파생으로** 찾아 전수 검사한다 — 목록형 금지", () => {
    ...
    const closure = importClosure(mapScreens, {
      // ★하한은 **모든 회귀 상태보다 위**여야 한다. 회귀별 폐포 실측: 별칭 전용·전깊이 143 ·
      //   깊이3 절단 155 · 깊이2 절단 140 · 별칭 전용·1단계 99 · 구판 62 · 상대유지 .tsx만 84.
      //   깊이 절단은 minDepth 가 지므로, 개수 하한이 져야 하는 최대 회귀값은 **143**이다.
      //   (초판 80 → 회귀 다수 통과. R1 의 130 도 143 을 통과시켰다 — 근거 수치를 잘못 적었다.)
      minFiles: 150,
    ...
    for (const hit of collected) {
      ...
      // ★`zs.length` 이 아니라 `missingZ` 로 본다 — 준수하는 갈래의 z 가 **z 없는 갈래를
      //   가리는** masking 이 있었다(R2 실측).
      if (hit.missingZ)
        violations.push(`${hit.file} — 백드롭 갈래에 z 유틸이 없다: ${hit.classes.slice(0, 70)}`);
    ...
    // ★정직 표기: 이 단언 때문에 **판정불가도 결국 테스트를 깨뜨린다** — 분리의 효익은
    //   "정상 코드를 통과시킨다"가 아니라 **진단이 정확해진다**는 것이다(위반이라고 거짓
    //   보고하지 않는다). 기준선을 0 으로 두는 이유는 판정불가가 조용히 늘면 그만큼 감시망이
    //   비기 때문이다(R2 M-D 지적 — 초판 주석이 효익을 과장했다).
    expect(
      undecidable,
  ...
  // ★R2 M-B: 삼항 갈래 분리는 `?`·`:` 가 **따옴표 밖**일 때만 동작한다. 템플릿은 한 조각이라
  //   그 안의 삼항은 여전히 병합돼, 비백드롭 갈래의 z 가 위반으로 신고되는 위양성이 남는다
  //   (`className={`${open ? "fixed inset-0 z-[800]" : "hidden z-10"}`}`). 저장소 현황 0건이라
  //   오늘은 무해하지만, 고친 척하지 않도록 부채로 드러낸다.
  it.todo(
    "★삼항 위양성이 **템플릿 안**에서는 잔존한다 — 갈래 분리가 따옴표 밖 `?`·`:` 에만 적용된다(현황 0건)",
  );
