...
describe("앱 전역 층위 사다리", () => {
  ...
  //     (`hooks/useMapFullscreen.ts` 의 `wrapperClass()` 를 소비하는 3곳). 그 전체화면 경로는
  //     별도 테스트(위 appFullscreen 단언)가 값으로 잠근다. R2 까지 "현황 0건"이라 적은 것은
  //     **거짓**이었다(R3 실증).
  it.todo(
    "★수집 밖 경계: **비리터럴 className**(삼항·cn()·템플릿·상수 조립) 백드롭은 수집되지 않는다 — " +
      "폐포 실사례는 useMapFullscreen.wrapperClass 계열(전체화면 z-[9990]·별도 단언이 잠금)",
  );
