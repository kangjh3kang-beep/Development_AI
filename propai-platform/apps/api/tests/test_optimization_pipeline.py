...
- sobol_first_order: correlation-based 1차 근사 — 완전상관/무상관 변수 구분.
"""
...
class TestSobolFirstOrder:
    def test_완전상관_변수는_지수_1에_근접(self):
        ...
        result = sobol_first_order({"x1": x1}, y)
        assert result["indices"]["x1"] == pytest.approx(1.0, abs=1e-6)
        assert "correlation_squared_proxy" in result["method"]
    ...
    def test_무관계_변수는_지수_0에_근접(self):
        ...
        result = sobol_first_order({"x": x_unrelated}, y)
        assert result["indices"]["x"] < 0.05

    def test_표본부족시_정직_사유(self):
        result = sobol_first_order({"x": [1.0]}, [1.0])
