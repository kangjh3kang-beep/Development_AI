...
#
# ★★생존 사유를 **항목이 아니라 클래스로** 적는다(2026-09-08 정정).
#   종전에는 생존을 하나씩 나열했는데, 그러면 **다음 실행에서 새 항목이 나올 때마다 목록을
#   늘려야 하고 그 자체가 손 목록**이다(이 PR 이 다섯 번 지적받은 바로 그 형태).
#   현재 생존은 아래 **두 클래스**로 남김없이 설명된다 — 개수가 아니라 **클래스**를 검증하라:
#
#     (C1) **내부 식별자**(`SCOPE_*` 등) — 고유한 다른 이름은 결함이 아니다. 계약은 «상호 구별».
#     (C2) **주제 밖 산문** — `subject_consistency.conflicts[].note` · `reference_note` 처럼
#          계수의 출처와 무관한 설명문. 일반 치환 변이는 마크다운도 출처 주장도 만들지 않으므로
#          위 (나-1)·(나-2) 가드를 비껴간다. ★이것이 **선언한 (다) 잔여**이고
#          `test_all_user_facing_strings_are_pinned` 가 **xfail(strict) 로 초록 안에 드러낸다.**
#          76개 문자열을 전부 동결하는 것은 과잉이라 하지 않았다(§A-6).
#
#   ⇒ 새 생존이 나오면 **먼저 이 두 클래스에 속하는지** 보라. 속하지 않으면 **진짜 구멍**이다.
# ─────────────────────────────────────────────────────────────
