...
def test_grade_is_withheld_below_coverage_floor() -> None:
    ...
    assert r.get("grade_withheld_reason"), "등급을 보류했으면 **사유**를 말해야 한다"
    assert str(r["covered"]) in r["grade_withheld_reason"], (
...
def test_grade_is_issued_at_or_above_floor() -> None:
    ...
    assert r.get("grade_withheld_reason") is None, "발행했는데 보류 사유가 남아 있다"
...
def test_both_gradeless_paths_have_the_same_key_shape() -> None:
    ...
    empty = _score({})
    low = _score(_ONE)
    for k in ("grade", "grade_withheld_reason", "covered", "total_features"):
    ...
    assert empty["grade_withheld_reason"] and low["grade_withheld_reason"]
