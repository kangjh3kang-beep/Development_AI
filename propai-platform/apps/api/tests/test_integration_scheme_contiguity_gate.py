...
★★**도구 함정 — 이 파일을 변이 검증할 때 반드시 읽을 것**

`scripts/mutate_manual.sh` 의 판정을 **`grep -oE "CAUGHT|SURVIVED" | head -1`** 로 긁으면
**거짓 SURVIVED** 가 난다. pytest 가 실패 테스트의 **소스 문맥을 출력**하는데, 이 파일 주석에
`CAUGHT`·`SURVIVED` 라는 낱말이 들어 있어 **도구의 판정 줄보다 먼저** 잡히기 때문이다
(실측 2026-09-02: 도구는 83행에 `CAUGHT` 를 찍었는데 34행의 **내 주석**이 먼저 잡혔다).

    ✘  scripts/mutate_manual.sh … | grep -oE "CAUGHT|SURVIVED" | head -1
    ✔  scripts/mutate_manual.sh … > /tmp/m.txt 2>&1; rc=$?
       grep -oE '^(CAUGHT|SURVIVED)' /tmp/m.txt | tail -1     # 줄 시작 앵커 + 마지막 줄
       # rc 도 함께 본다(0=SURVIVED · 1=CAUGHT · 10=미커밋 · 11=주입실패)

= CLAUDE.md §검증 규율 8 *"가장 자주 틀리는 상대는 코드가 아니라 **자기가 방금 쓴 텍스트**다"*
  의 재현이다. 이 주석 자체도 그 원천이므로 **앵커 없이 긁지 마라.**
"""
import ast
import inspect
import pathlib
...
def test_absorption_rule_prevents_over_restriction():
    ...
    **그 밖의 건축 제한 등에 관한 사항은 그 대지 중 가장 넓은 면적이 속하는 용도지역등에 관한
    ...
    #   흡수 대상이 유일한 제한 지역이면 두 구현(`filter` / `= []`)의 답이 같으므로,
    #   **흡수되는 쪽이 제한 대상이 아닌** 조합과 **둘 다 제한 대상인** 조합을 함께 둔다.
    #   ★기록 정정: 이 케이스들을 추가한 계기는 「`restricted = []` 변이가 SURVIVED」였는데,
    #     그 판정은 **거짓**이었다(아래 「도구 함정」 참조). 케이스 자체는 유효하므로 남긴다 —
    #     두 모집단을 실제로 가르기 때문이다.
