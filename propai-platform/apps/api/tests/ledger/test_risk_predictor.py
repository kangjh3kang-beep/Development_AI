...
from app.services.ledger import risk_predictor as P

pytestmark = pytest.mark.asyncio


def test_extract_features_pure():
    feats = P.extract_features(
...
def test_predict_heuristic_when_no_model():
    hi = P.predict_risk_score({"version_count": 5.0, "current_risk_level": 3.0, "risk_signal_count": 3.0})
    lo = P.predict_risk_score({"version_count": 1.0, "current_risk_level": 0.0, "risk_signal_count": 0.0})
    assert hi["model"] == "heuristic" and hi["score"] > lo["score"]
    assert hi["level"] == "high" and lo["level"] == "low"
    # 빈 feature도 결정론·예외 없음
    empty = P.predict_risk_score(P.extract_features(history=[], latest_risk=None))
    assert empty["model"] == "heuristic" and 0.0 <= empty["score"] <= 1.0


def test_load_model_graceful_none():
    assert P._load_model() is None   # xgboost/모델 부재 → None(휴리스틱 폴백)
...
async def test_predict_chain_risk_integration():
    ...
    try:
        ...
        pred = await P.predict_chain_risk(analysis_type=at, tenant_id=tid, pnu=pnu)
