...
★현재는 게이트(`down_pct >= 40.0`)가 라이브에서 미달이라 **잠복**이다. 잠복이 곧 무해가
아니다 — 게이트가 넘는 날 그 인사이트는 **영원히 `open`** 이 된다.
...
★★**기준을 달아 적는다**(종전엔 *"관측: …4건 전부 down_pct=0.0"* 이 **측정 시각·주체 없이**
  적혀 있었다). 휘발성 값은 **명령과 기준**을 함께 적지 않으면 조용히 낡는다 —
  이 저장소가 `effector_firing.py:16` 의 *"★66시간 휴면"*(재측정 ~447시간)에서 이미 겪었다.
