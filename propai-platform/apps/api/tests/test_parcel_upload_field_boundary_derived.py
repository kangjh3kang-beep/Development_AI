...
엑셀 업로드 사슬:
    ...
    ① 양식 다운로드  GET  /zoning/land-schedule-template → `build_template_xlsx()`
       ★2026-09-12 정정: 종전 근거는 *"안내 시트가 … 활용됩니다 라고 **약속**한다"* 였는데
       그 문장은 양식 축소(#1044)로 **삭제됐다**(`grep 활용됩니다` → 생산 파일 0건).
       ⇒ 이 락의 근거는 그 약속이 아니라 **일반 원리**다: ***파서가 응답에 싣는 필드가
       타입 경계에서 조용히 버려지면 안 된다.*** 근거가 사라진 락을 그대로 두면
       다음 사람이 확인하러 갔다가 **없는 문장**을 찾게 된다.
...
def test_파서가_내는_필드는_프론트_타입에_선언돼야_한다(field: str):
    ...
    ★`consent_*` 는 종전에 *"활용됩니다"* 라고 **약속한** 칸이었다 — 그 문장은 #1044 에서
    삭제됐고(표준 양식이 주소·지번 두 칸으로 줄었다), 지금은 **사용자가 이미 쓰던 자기 양식**에
    그 칸이 있을 때 읽는다. **근거가 「약속」에서 「파서가 실제로 싣는 값」으로 바뀌었다.**
