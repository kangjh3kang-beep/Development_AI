...
class HubPermitClient(BaseAPIClient):
    ...
    async def _get_data(
        self, endpoint: str, params: dict[str, Any], cache_key: str, cache_ttl: int = 86400
    ) -> list[dict[str, Any]]:
        """공통 API 호출 메서드."""
        # 기본 공통 파라미터
        base_params = {
    ...
    async def get_expos_pubuse_area_info(self, sigungu_cd: str, bjdong_cd: str = "") -> list[dict[str, Any]]:
        ...
        if bjdong_cd:
            params["bjdongCd"] = bjdong_cd
        return await self._get_data(
            "getHpExposPubuseAreaInfo", params, cache_key=f"hub_permit:expos:{sigungu_cd}:{bjdong_cd}"
        )
...
class ArchPermitClient(BaseAPIClient):
    ...
    async def get_ap_atch_pklot_info(self, sigungu_cd: str, bjdong_cd: str = "") -> list[dict[str, Any]]:
        ...
        if bjdong_cd:
            params["bjdongCd"] = bjdong_cd
        return await self._get_data(
            "getApAtchPklotInfo", params, cache_key=f"arch_permit:atch_pklot:{sigungu_cd}:{bjdong_cd}"
        )
    ...
    async def get_ap_expos_pubuse_area_info(self, sigungu_cd: str, bjdong_cd: str = "") -> list[dict[str, Any]]:
        ...
        if bjdong_cd:
            params["bjdongCd"] = bjdong_cd
        return await self._get_data(
            "getApExposPubuseAreaInfo", params, cache_key=f"arch_permit:expos:{sigungu_cd}:{bjdong_cd}"
        )
