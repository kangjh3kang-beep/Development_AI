...
#:
#: ★★2026-08-27 — 종전 이 자리에 *"MOLIT 이 안 준다는 것은 **추론**이다. 원천 응답
#:   원문의 키 목록은 확인하지 않았다"* 라고 적혀 있었다. **그 원문을 확인했다.**
#:   이제 **관측**이다(증거 규율 §1 — 추론에서 승격):
#:
#:       라이브 원문 직접 조회 2026-08-27T00:3xZ · 활성 컨테이너 propai-api-8000
#:       MolitClient._request(LAWD_CD=11680, DEAL_YMD=202606, _type=json)
#:       — **파서를 거치지 않은 응답 item 의 키 집합**
#:
#:         land  65건 → **16키**  rgstDate ✘ · buyerGbn ✘ · slerGbn ✘
#:         apt  200건 → **32키**  rgstDate ◎ · buyerGbn ◎ · slerGbn ◎
#:
#:   **land 원문에는 그 세 키가 아예 없다**(값이 0/공백인 것이 아니라 **부재**).
#:   ★대조군: `cdealType`·`estateAgentSggNm` 은 **양쪽 다 존재**한다 → 조회기와
#:     파서가 살아 있음이 같은 실행에서 증명된다(대조군 없는 0건은 근거가 아니다).
#:
#: ★**왜 한 번 헛짚었는지 남긴다**: 먼저 `client.get_transactions()` 로 재니 land·apt 의
#:   키 집합이 **20개로 완전히 동일**했다 — 파서가 **고정 스키마로 정규화**하기 때문이다.
#:   그대로 봤으면 *"토지도 등기 필드를 받는데 값만 빈다"* 로 정반대 결론이 났다.
#:   **도구 출력이 원문과 다르면 원문이 옳다**(증거 규율 §6).
#:
#:   → 토지를 꼬리에 넣으면 **잡을 것이 없는 요청을 매주 24회** 낸다(순수 쿼터 낭비).
#:   ★★그리고 이것은 수집 창의 문제가 아니라 **API 계약**이므로, 창을 아무리 넓혀도
#:     토지 등기는 오지 않는다 — **표면③(등기부분석에 실거래 소유권 이력)은 토지에서
#:     원천 불가**다. 토지에서 쓸 수 있는 신호는 `cdealType`/`cdealDay`(해제) ·
#:     `shareDealingType`(지분) · `jimok` · `landUse` 다.
