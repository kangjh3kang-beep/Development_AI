...
- 조례값의 정직한 등급(★핵심 설계결정): ordinance_service.ORDINANCE_CACHE는 정적 캐시라
  W2-1 SourceSnapshot이 없다(SourceSnapshot은 VWorld·G2B 두 커넥터만 opt-in — source_snapshot.py
  모듈독스트링 참고). "스냅샷이 없다"는 사실을 SNAPSHOT으로 거짓 격상하지 않고, 별도(더 낮은)
  등급 STATIC_CACHE로 정직하게 표현한다 — "출처는 있으나 원본 바이트까지 사슬이 닿지 않는다"는
  뜻이다. 법제처 API(법정상한이 아닌 실시간 조회) 역시 아직 SourceSnapshot 미연동(W2-1은
  VWorld·G2B만 opt-in)이므로 SNAPSHOT을 자칭하지 않고 UNKNOWN(미추적)으로 정직 하강한다
  (한계·후속 — MOLEG 커넥터의 SourceSnapshot opt-in은 W2-3 이후 과제).
"""
...
# 계보 등급(source_kind) 5종 + UNKNOWN — 값이 사슬의 어느 단계에서 왔는지.
#   SNAPSHOT     : SourceSnapshot(W2-1) 1건에 연결됨 — 원본 바이트까지 재현 가능.
...
LINEAGE_SOURCE_KINDS: frozenset[str] = frozenset({
    "SNAPSHOT", "STATIC_CACHE", "CALC", "RULE", "USER_INPUT", "UNKNOWN",
})
...
class LineageRef:
    ...
    def traced(self) -> bool:
        ...
        ★불변식(스펙 원문): "UNKNOWN이면 False, 그 외(SNAPSHOT/STATIC_CACHE/CALC/RULE/
        USER_INPUT)는 True" — 스냅샷이 없어도(STATIC_CACHE) '출처가 정직하게 명시'되어
        있으면 추적된 것으로 본다. 완전히 미상(UNKNOWN)일 때만 미추적이다.
        """
