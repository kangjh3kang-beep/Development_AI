"""통합 세금 엔진 — 28종 4단계 일괄 오케스트레이션.

취득(A01~A10) + 공사(B01~B04) + 분양(C01~C08) + 양도(D01~D06) = **28종**.

★2026-08-28 교정 — 종전 헤더는 **같은 줄 안에서 자기모순**이었다: 코드 목록은 `B01~B04` 로
  갱신됐는데 합계는 `32종 기본 + 조건부 6종 = 38종` 인 채였다(실제로는 10+4+8+6 = **28**).
  ★★이 줄 자체가 한 번 썩었다(2026-08-28) — `#926` 에서 이 파일에 `38종`→`28종` **일괄 치환**을
    걸었는데, **옛 값을 인용한 이 문장까지 치환**돼 «32+6=28» 이라는 헛소리가 됐다.
    **일괄 치환은 「선언문」과 「인용문」을 구별하지 못한다** — 결함을 설명하는 문장이
    그 결함을 그대로 겪었다. 다음에 이 파일에 일괄 치환을 걸 사람은 이 줄을 먼저 보라.
  · `38` 은 애초에 파생되지 않는 수였다(«조건부 6종» 은 별도 코드가 아니라 **기존 코드 안의
    조건부 항목**이다 — 실측: `"code": "[A-D]dd"` 전수 28개, 그 밖의 표기 0건).
  · `32 → 28` 은 `#913` 이 인입 4종(B05 전기·B06 가스·B07 통신·B08 소방)을
    **부담금에서 공사비로 재분류**하면서 이 디렉토리를 떠났기 때문이다.
★이 수치는 화면 문구와 결속돼 있다 —
  `apps/api/tests/test_ui_capability_claims_are_backed_by_code.py` 가 여기서 **파생**해
  i18n 3개 언어의 주장을 잠근다. **여기를 고치면 문구가 빨개진다.**
"""
...
def get_applicable_tax_codes(
    *,
    ...
    # 공사단계 (항상)
    base_codes.extend(["B01", "B02", "B03", "B04"])  # ★B05~B08 제거(2026-08-27): 부담금이 아니라 공사비
