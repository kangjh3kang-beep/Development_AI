...
_H_AREA = {"면적", "면적㎡", "면적m2", "토지면적", "area", "areasqm", "대지면적", "공부면적",
           "지적면적", "실면적", "면적m²"}
...
def build_template_xlsx() -> bytes:
    ...
    ws.append(["예시2(삭제)", "서울특별시 강남구 역삼동 737", "737", "1168010100", "", "", "",
               "사유", "O", "X", "X", "← 예시행(삭제 후 작성)"])
    for r in (2, 3):
...
class ParcelExcelService:
    ...
    async def parse(self, raw: bytes, filename: str) -> dict[str, Any]:
        ...
        try:
            if name.endswith(".csv"):
                ...
                for enc in ("utf-8-sig", "cp949", "euc-kr", "latin1"):
                    try:
                        ...
                        continue
                if df0 is None:
                    return {"error": "CSV 인코딩을 인식하지 못했습니다(UTF-8/CP949). 엑셀(.xlsx)로 저장해 올려보세요.",
                            "parcels": []}
            ...
            return {"error": f"엑셀/CSV를 읽지 못했습니다: {str(e)[:120]} — 엑셀(.xlsx)로 저장해 다시 시도해 주세요.",
                    "parcels": []}
        ...
        if not cols["address"] and not cols["pnu"] and not cols["bcode"]:
            return {
                "error": ("필수 컬럼을 찾지 못했습니다 — 최소 [소재지(주소)] 또는 [PNU] 또는 [법정동코드]가 필요합니다."
                          " 표준 양식을 내려받아 작성하거나, 헤더에 '소재지/지번/면적' 등을 명시해 주세요."),
