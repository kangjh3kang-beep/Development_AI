...
#: 양식 시트에 찍히는 열. ★**주소와 지번 둘뿐이다**(2026-09-12).
#:
#: 종전 양식은 12열이었고 그중 **10열이 「비워도 되는」 칸**이었다(법정동코드·PNU·지목·면적·
#: 소유구분·동의 3종·연번·비고). 안내문도 *"[소재지(주소)] 는 필수입니다. 나머지는 비워도
#: 됩니다"* 라고 적고 있었다 — ***양식이 스스로 「대부분 안 채워도 된다」고 말하면서 그 칸들을
#: 사용자 앞에 늘어놓고 있었다.***
#:
#: ★**열을 줄여도 업로드 능력은 줄지 않는다**(실측 2026-09-12): 파서는 헤더를 **별칭 집합**
#:   (`_H_ADDR`·`_H_JIBUN`·`_H_PNU`·`_H_CONSENT_*` …)으로 인식하므로, 사용자가 자기 파일에
#:   그 열을 그대로 두면 **여전히 읽는다.** 바뀐 것은 *양식이 무엇을 요구하는가* 뿐이다.
#:   대조군으로 12열 파일을 그대로 태워 확인했다(필지 추출 정상).
#: ★튜플이 아니라 **이름만** 둔다(2026-09-12 · 독립 리뷰 MINOR-7).
#:   예시행을 없앤 뒤 두 번째 원소(예시값)는 **소비처 0** 이 됐다 — `("지번","224")` 를
#:   `("지번","ZZZ딴값")` 으로 바꿔도 **전부 초록**이었다(변이 SURVIVED 실측).
#:   죽은 데이터를 남기면 안내문의 예시와 **조용히 갈린다.**
TEMPLATE_COLUMNS = ["소재지(주소)", "지번"]
...
def build_template_xlsx() -> bytes:
    ...
    # ★열 너비 — **입력할 값의 길이**로 정한다(머리글 길이가 아니다 · 독립 리뷰 MINOR-8).
    #   종전 `max(12, len(name)+4)` 는 두 열 다 **12** 였는데 주소는 20자가 예사다.
    #   양식 사용성을 고치는 변경이 정작 **적는 칸**을 좁게 둔 자리였다.
    _WIDTH = {"소재지(주소)": 42, "지번": 14}
    for col, name in enumerate(TEMPLATE_COLUMNS, start=1):
        c = ws.cell(row=1, column=col, value=name)
        c.fill = head_fill
        c.font = head_font
        c.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
        ws.column_dimensions[c.column_letter].width = _WIDTH.get(name, max(12, len(name) + 4))
    # ★**예시행을 데이터 시트에 넣지 않는다**(2026-09-12).
    #   종전엔 2·3행에 예시를 깔고 안내문이 *"삭제 후 작성"* 이라고 적었는데, 이 저장소에는
    #   **예시값이 지워지지 않은 채 업로드돼 면적이 오적재된 전례**가 있다(그래서 면적·지목
    #   칸을 비워 두는 완화까지 들어가 있었다). 열이 둘로 줄어든 지금은 예시행이 남으면
    #   **존재하지 않는 필지 2건이 그대로 등록**된다 — 완화가 아니라 **원인을 없앤다.**
    #   ⇒ 데이터 시트는 **머리글만** 두고, 예시는 「작성안내」 시트에서 보여 준다.
    ws.freeze_panes = "A2"          # 행이 늘어도 머리글이 보인다

    # 안내 시트.
    g = wb.create_sheet("작성안내")
    # ★★사용자 대면 셀에는 **마크다운을 쓰지 않는다**(독립 리뷰 MEDIUM-4).
    #   엑셀은 `**` 를 굵게 렌더하지 않고 **별표 그대로** 찍는다. 이전 판에 6셀이 그랬다.
    #   강조가 필요하면 `Font(bold=True)` 를 쓴다(아래에서 머리 줄에만 적용).
    # ★★그리고 이 시트의 **어떤 줄도 「주소로 읽히면 안 된다」**(독립 리뷰 MEDIUM-3).
    #   엑셀의 «다른 이름으로 저장 → CSV» 는 **활성 시트만** 내보내므로, 사용자가 이 시트를
    #   활성화한 채 CSV 로 저장해 올리면 이 시트가 **데이터로 파싱된다**. 이전 판은 그때
    #   실제 주소 2건을 포함해 **21건이 필지로 등록**됐다(종전 양식은 정상 주소 0건이었다 —
    #   즉 이 PR 이 새로 심은 위험이었다). 그래서 예시 주소를 **괄호 안에** 두고
    #   머리글로 읽힐 표 모양(`소재지(주소) | 지번`)을 **두지 않는다**.
    #   락 `test_guide_sheet_parsed_as_data_yields_zero_parcels` 가 이것을 지킨다.
    notes = [
        "PropAI 다필지 토지조서 업로드 양식",
        "",
        "[토지조서] 시트에 두 칸만 채워서 올리면 됩니다 — 소재지(주소), 그리고 지번.",
        "나머지(법정동코드·PNU·지목·면적)는 자동으로 조회합니다.",
        "",
        "1) 소재지(주소) — 필수입니다. 시·도부터 번지까지 적어 주세요.",
        "     (보기: 경기도 의정부시 의정부동 224)",
        "     (보기: 경기도 안양시 동안구 비산동 511-168)",
        "2) 지번 — 주소에 번지가 없을 때 여기에 적어 주세요. (보기: 224, 511-168)",
        "     주소를 번지까지 적으셨다면 이 칸은 비워 두셔도 결과가 같습니다.",
        "     (주소에 이미 들어 있는 지번은 조회 질의를 바꾸지 않습니다.)",
        "3) '산' 지번은 '산12-3' 처럼 '산' 을 붙여 적으세요.",
        f"4) 한 번에 최대 {_MAX_ROWS}필지까지 업로드됩니다.",
        "",
        "[토지조서] 시트에는 예시행을 넣어 두지 않았습니다.",
        "예시를 지우지 않고 올려서 없는 필지가 등록되는 일을 없애기 위해서입니다.",
        "",
        "이미 쓰시던 양식이 있다면 그대로 올리셔도 됩니다.",
        "PNU(19자리)·법정동코드(10자리)·지목·면적·소유구분·동의 칸이 있는 파일도 그대로 읽습니다.",
        "(열 이름을 알아서 찾습니다 — 이 양식으로 옮겨 적지 않으셔도 됩니다.)",
        " · PNU 가 있으면 가장 정확하고, 없으면 법정동코드+지번, 그것도 없으면 주소로 조회합니다.",
        " · 면적을 직접 적으면 공부상(VWorld) 면적과 대조해 1.5배 이상 차이 나면 공부상으로 "
        "보정하고 경고합니다.",
        " · 소유구분은 자동으로 조회하지 않습니다. 파일에 그 칸이 있으면 읽고, 없으면 비워 둡니다.",
        " · 동의율(소유자 동의 비율) 자동산정은 아직 제공하지 않습니다 — 소유자별 동의를 적을 "
        "칸이 없어 공유지분 필지에서 분자가 과대해지기 때문입니다(소유자 단위 입력 도입 후 연결 예정).",
    ]
