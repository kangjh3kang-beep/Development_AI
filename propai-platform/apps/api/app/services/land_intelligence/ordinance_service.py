...
def _flex_zone_pattern(zone: str) -> str:
    ...
    ★**줄바꿈은 허용하지 않는다**(`\s*` 가 아니라 수평 공백 전용). `\s*` 로 두면 이름이
    줄을 넘어 이어져도 매칭돼, 앞 줄 끝 글자와 다음 줄 첫 글자가 우연히 이어지면
    **없는 용도지역을 지어낼** 수 있다. 이 저장소의 교훈상 **틀린 값은 `None` 보다 위험하다**
    (2026-08-19: 자연녹지 bcr 30 이 신뢰도 0.95 로 나와 그럴듯하게 통과할 뻔했다).
    실측 근거 — 오산시 조례 원문에서 `\s*` 와 수평공백 전용의 매칭 수가 **21개 용도지역
    전부 동일**했다(줄을 넘는 이름 0건). 즉 조이는 비용이 **0** 이다.
    공백류 분포도 쟀다: U+0020 6,874 · U+000A 532 (탭·NBSP·전각공백 0) —
    다른 지자체를 위해 전각공백/NBSP 는 **허용**하되 줄바꿈만 뺀다.
