"""토지 적정 매입가 추정 — 공시지가 × 지역 시세보정계수.

★**정정(2026-09-15)**: 이 줄은 «(+ 주변 토지 실거래 블렌딩)» 이라 적고 있었는데 **거짓**이었다.
  같은 파일 `:175` 가 *"단일 출처(공시지가 기준) 추정입니다"* 라고 정반대를 적고 있었고,
  이 모듈에 실거래 조회는 **0건**이다(`molit`·`transaction` 심볼 부재로 확인).
  ***두 표면이 모순할 때 더 구체적으로 들리는 쪽이 거짓이었다*** — 그리고 첫 줄은 맨 위에
  있어 가장 먼저 읽힌다. 다음 사람은 «실거래가 반영된다» 고 믿고 재검증하지 않는다.
