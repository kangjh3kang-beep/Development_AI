...
def _zone_match_clause(land_stats: dict | None, target_land_use: str | None) -> str:
    ...
    #   ★대신 **계약은 전부 잠갔다**: ①일치율 수치가 실린다 ②100% 일치인데 「나머지는」이라
    ...
    *,
    ...
    if comparable_avg_per_sqm is None and lawd_cd_from_pnu(pnu):
        try:
            ...
            # ★★2026-09-07 사용자 신고 — *"시세분석이 시장가를 못 잡는다"* ·
            #   *"실거래 기준일에 따라 천차만별인데 반영해야 하지 않나"*.
            #   라이브 실측(남양주 화도읍 마석우리 265-1 · 일반상업지역):
            #     선택된 층 `sigungu_jimok`(시군구·**지목=답**) 102건 → 732,245원/㎡
            #       └ 그 표본의 용도지역 구성 **개발제한구역 40.2%** — 대상은 일반상업지역
            #     같은 용도지역 실거래 중앙(12개월) **4,038,261원/㎡**
            #     채택가 3,057,000 → 실거래 대비 **−24%**
            #
            #   ★원인은 «용도지역 층이 없어서»가 **아니다** — 사다리에 `dong_zone`·
            #     `sigungu_zone` 이 **이미 있다.** **창이 좁아 표본이 안 차서** 못 선다:
            #       6개월 0건 · 12개월 3건 (MIN_SAMPLE=5)
            #
            #   ★★그리고 창을 넓히면 **시점수정이 필요해진다.** `nearby_map_service` 는
            #     *"창이 6개월이라 보정 없이도 왜곡이 작다(**30개월 창을 쓸 때 다시 판단할
            #     것**)"* 라고 **예고**해 두었다. 실측으로 그 판단이 옳음을 확인했다:
            #       창 6개월 시점수정 효과 **+0.55%** ↔ 용도지역 불일치 **−24%**(두 자릿수 차)
            #     → 그래서 **24개월 창 확대를 실제로 만들어 라이브로 재 봤고, 기각했다**:
            #         6개월  층=sigungu_jimok n=85  786,502원/㎡
            #         24개월 층=sigungu_jimok n=601 **631,272원/㎡**  ← 층은 그대로, 값만 −20%
            #       창이 아니라 **매칭**이 막고 있었다(아래 `_matches` 읍·면 봉합 참조).
            #       사용자 판단도 같다 — *"실거래 기준일은 6개월 이내여야 실효성이 있다"*.
