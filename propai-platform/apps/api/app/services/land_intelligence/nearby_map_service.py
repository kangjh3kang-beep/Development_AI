    ...
    async def build(
        ...
        for cat in categories.values():
            ...
            cat["precut"] = {
                ...
                #   송동리·제내리, 송도동, 효자동, 일월동 뿐). 즉 이 조합은 **세 원인**을 포함한다:
                ...
                #                            표기다. 행정동 `"길음1동"` vs 법정동 `"길음동"`,
                #                            `"우1동"` vs `"우동"` 처럼 주소가 행정동 표기면
                #                            `_dong_from_address` 가 그대로 뽑아 온다.
                #                            수정 지점은 `_dong_from_address`(D-1 계열). → **조사 대상**
                #   ★리뷰 차단 봉합 — 정정 초판은 (3)을 빠뜨렸고, "대상 동이 어떤 형태로도 없으면
                #     (2)"라는 규칙이 (3)을 **정상으로 닫아버렸다**. (3)의 관측 서명은 (2)와
                #     겹치는데(대상 동이 관측 목록에 없다) 의미는 정반대다 — 프라이어가 전 그룹에서
                #     무음으로 꺼진 상태다. 종전의 "전부 경보"는 이 케이스를 **우연히 올바르게**
                #     조사로 보냈으므로, 초판 규칙은 탐지력 기준 **국소 회귀**였다.
                #     거짓양성보다 거짓음성이 나쁘다는 이 저장소의 서열(W2-b·#497·W2-c)을 따른다.
                #   가르는 법(순서대로):
                #     ① `groups_before == 0` 이면 **카테고리 전체 무자료**다(즉답 — 동 문제 아님).
                #     ② 아니면 `target_dong_hint` 를 관측된 `dong` 분포와 대조한다:
                #        같은 동의 **다른 표기**가 보이면 (1) · **유사하지만 다른 이름**이 보이면 (3)
                #        · **무관한 동만** 보이면 (2).
