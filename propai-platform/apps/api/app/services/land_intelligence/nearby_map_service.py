    ...
    async def build(
        ...
        for cat in categories.values():
            ...
            cat["precut"] = {
                ...
                #   송동리·제내리, 송도동, 효자동, 일월동 뿐). 즉 이 조합은 두 원인을 **모두**
                #   포함한다:
                #     (1) 표기 규약 불일치 — 우리 정규화가 `umdNm` 형태를 못 따라간다(조사 대상)
                #     (2) 대상 동 무자료   — 그 동에 해당 카테고리 거래가 없다(**정상**)
                #   가르는 법: 응답의 그룹 `dong` 분포에 대상 동이 **어떤 형태로도** 없으면 (2)다.
                #   ★부분일치 카운터를 하나 더 실어 자동 분리하는 안은 **기각**했다 —
                #     `"중동"` 이 `"중동리"` 에 부분일치하는 식의 위양성을 새로 만들고, 그러면
                #     이 주석이 고치려는 오독을 **다른 형태로 재생산**한다(관측 장치가 결함
