    ...
    async def build(
        ...
        for cat in categories.values():
            ...
            cat["precut"] = {
                ...
                #   송동리·제내리, 송도동, 효자동, 일월동 뿐). 즉 이 조합은 **네 원인**을 포함한다:
                ...
                #   가르는 법(순서대로 — **중간에 멈추지 말 것**):
                #     ① `groups_before == 0` 이면 **카테고리 전체 무자료**다(즉답 — 동 문제 아님).
                #     ② `sigungu_source`/`sigungu_hint` 가 대상지 시군구와 맞는가? 아니면 **(4)**.
                #     ③ `target_dong_hint` 가 **법정동인가**? 행정동(숫자 접미·병합명)이면 관할
                #        법정동과 이름이 안 겹치는 것이 **정상 동작**이므로 **(3)** 이다.
                #        ★이 확인을 건너뛰고 ④로 가면 (3b)를 (2)로 오분류한다.
                #     ④ 법정동임이 확인된 뒤에만 관측 `dong` 분포와 대조한다:
                #        같은 동의 **다른 표기**가 보이면 (1) · **무관한 동만** 보이면 (2).
