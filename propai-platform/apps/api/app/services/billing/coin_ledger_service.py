...
ENTRY_TYPES = (
    ...
    "admin_adjust",   # 관리자 조정(±)
    # ── 환불(PG) ──────────────────────────────────────────────────────────
    # ★이 두 줄이 없으면 `append_event` 가 **조용히 건너뛴다**(`persisted: False` 를
    #   돌려줄 뿐 예외를 안 던진다 — :175). 그러면 **돈은 움직이고 원장은 빈다.**
    #   정합성 렌즈가 실측으로 잡은 결함이다(2026-08-27).
    "order_refunded",           # 결제 환불로 코인 환수(-)
    "order_refund_reverted",    # 벤더 취소 실패로 환수 원복(+)
)
