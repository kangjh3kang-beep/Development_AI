...
#: ★**포함 범위(scope)** — 이 단가가 **무엇을 덮는가**. 2026-08-27 이전에는 어디에도
#: 적혀 있지 않았고, 그래서 *"소방설비가 여기 포함되는가"* 를 아무도 답할 수 없었다.
#: `#913` 이 소방 항목(3,500원/㎡)을 **이중계상으로 제거**한 근거가 바로 이 질문이었다.
#:
#: **포함(의도)**: 직접공사비 전체 — 골조·조적·방수·창호·마감 +
#:          **기계설비(소화설비 포함)** + **전기설비(소방전기·통신 포함)**.
#: **제외(의도)**: 간접공사비(설계·감리·예비비·일반관리비 — `calculate_indirect_cost` 별도) ·
#:          토지비 · 금융비 · 제세금 · **공급자 인입 분담금**(한전·도시가스사·통신사 —
#:          도급 밖이라 `cost/utility_connection_cost.py` 가 별도 계상).
#:
#: ★★**근거의 강도를 정직하게 적는다 — 증명이 아니다.**
#:   초안은 「근거 세 축」이라고 썼는데 **독립 리뷰가 그중 둘을 무너뜨렸다.**
#:
#:   ◎ **살아남은 축 — 적산 실적 데이터**: `data/boq_master/mechanical.json` 의 level-1 절이
#:     「**소화설비공사**」이고 `electrical.json` 의 level-1 절이 「**소방공사**」다. 즉 실제
#:     공내역서에서 **소방은 기계·전기 공종 안에 있다.** 이것이 *"소방이 별도 부담금 항목이
#:     아니다"* 를 지지하는 **유일한 직접 증거**다.
#:
#:   ✘ **기각 — QTO 모델의 `mep_ratio`/`elec_ratio`**: 그것은 **QTO 산식의 계수**이지 이 단가의
#:     계수가 아니다. 두 산식은 별개이고, `overview_estimator` 가 **같은 저장소의 언어로**
#:     그 혼용을 금지한다(*"…와는 다른 축이다 … **혼용 금지**"*). **범주 오류였다.**
#:
#:   ✘ **기각 — 크기 비교**: 초안은 *"QTO(설비 포함) 549,704 < 이 단가 2,400,000 이므로
#:     설비를 빼고는 성립하지 않는다"* 고 썼다. **사실과 반대다**(2026-08-27 실측):
#:         설비(07+08) = 180,775원/㎡ = QTO 의 32.9% · **이 단가의 7.5%**
#:         배수: 설비 **포함** 4.37 vs 설비 **제외** 6.51  ← **빼면 오히려 커진다**
#:     간극을 만드는 것은 설비가 아니라 **QTO 가 모델링하지 않는 공종**(마감·토공·흙막이·
#:     승강기·조경)이다. 따라서 이 부등호는 **판별력이 0** 이다.
#:
#: ★**그러므로**: 위 「포함/제외」는 **증명된 사실이 아니라 이 저장소의 의도**이고,
#:   그것을 지지하는 직접 증거는 **실적 내역서 하나**다. 나머지는 **모순되지 않을 뿐**이다.
#:
#: ★**미측정**: 도급 계약서·공사비 원가계산서 같은 **1차 출처로 확인하지 못했다.**
#:   ★**트리거**: 1차 출처로 *"이 단가가 설비를 제외한다"* 가 확인되면 이 서술을 뒤집고
#:     **`#913` 의 소방 제거(−23,002,000)를 재검토**해야 한다.
#:   ★이 범위를 바꾸려면 `tests/test_direct_rate_scope_contract.py` 를 먼저 보라.
