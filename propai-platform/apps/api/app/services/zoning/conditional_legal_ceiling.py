...
def resolve_conditional_ceiling(
    ...
    return {
        ...
        "note": (
            f"성장관리계획구역이라 {zone}의 법정 건폐율 상한이 "
            f"{bcr}% 까지 열릴 수 있습니다(제77조제1항에도 불구하고). "
            "다만 실제 완화 폭은 성장관리계획 본문과 조례가 정하므로, "
            "그 두 가지를 확인하기 전까지는 적용값으로 쓰지 않습니다."
        ) if bcr is not None else (
            f"성장관리계획구역이라 {zone}의 법정 용적률 상한이 {far}% 까지 열릴 수 있습니다. "
            "실제 완화 폭은 성장관리계획 본문과 조례가 정합니다."
        ),
    }
