...
class OnbidClient:
    ...
    async def get_pbanc_cltr_list(
        self, pbanc_mng_no: str, rows: int = 50,
    ) -> dict[str, Any]:
        """공고 물건정보(getPbancCltrInf2). 필수 pbancMngNo. 공고 내 물건 리스트 반환.

        각 물건: 재산유형(prptDivNm)·처분방식(dspsMthodNm)·용도분류(cltrUsg*CtgrNm)·
        유찰횟수(usbdNft)·감정가(apslEvlAmt)·최저입찰가표시(lowstBidPrcIndctCont)·입찰기간.
        무자료/실패는 빈 리스트(가짜 금지).
        """
        if not self._service_key or not pbanc_mng_no:
        ...
        url = f"{ONBID_BASE_URL}/{ONBID_PBANC_CLTR_OP}"
        try:
            client = await self._get_client()
            resp = await client.get(url, params=params)
            resp.raise_for_status()
            raw_items, err = self._extract_items(resp.text)
            if err:
                return {"items": [], "data_source": "unavailable", "reason": err}
            return {"items": raw_items, "data_source": "onbid_live"}
        except Exception as e:  # noqa: BLE001
    ...
    def _extract_items(text: str) -> tuple[list[dict[str, Any]], str | None]:
        ...
        #     · SSRF(외부 URL)  → **거부됨**
        #     · **billion laughs(엔티티 폭탄) → 확장됨**(3단계로 1000자, 늘리면 기하급수)
        #   즉 표준 파서는 XXE 는 막아도 **엔티티 폭탄은 못 막는다** — 작은 응답이
