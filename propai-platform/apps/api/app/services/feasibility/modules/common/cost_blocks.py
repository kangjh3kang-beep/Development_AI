...
#
# ★★**결정됨(2026-09-06 · 사용자): A — 현행 유지.** 이중계상이 아니라 **경계 문제**였다:
#     공사비 간접비 = 설계비·감리비·예비비(설계변경)·일반관리비   ← 정본
#     소프트비 7%   = 분양대행·광고·금융수수료·제세공과 등 사업경비 ← 정본
#   **거짓은 계산이 아니라 주석이었다.** 값은 바꾸지 않는다.
#   ★기각한 대안: B)4% · C)3% 는 덮는 범위가 줄지 않는데 값만 깎는 것 ·
#     **D)공사비에서 그 셋 제거는 적산 보고서·API 가 그 셋을 읽어** 「설계비·감리비·예비비」
#     행이 **0원**이 된다(`report/render/cost_estimation_adapter.py:127-129` ·
#     `routers/cost.py:216-217`). **적산은 수지와 다른 질문에 답한다.**
#   경계는 `tests/test_soft_cost_double_count_ledger.py` 가 잠근다 — 어느 한쪽이 상대의
#   항목을 자기 것이라 주장하면 빨개진다.
