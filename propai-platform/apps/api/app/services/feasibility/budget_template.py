...
부담금 코드는 utility/sale stage 엔진과 정합한다(**B01~B04** = utility_stage, C07·C08 = sale_stage).
★2026-08-27: 종전 `B01~B08` 이라 적었으나 **B05~B08 은 부담금이 아니어서 엔진에서 빠졌다** —
  인입 3종은 「공사비」 그룹으로 이관(U01~U03), 소방은 도급단가 포함이라 제거(이중계상).
...
_COMMON: list[tuple[str, str, str]] = [
    ...
    # 인입공사비 — ★2026-08-27 「각종부담금」에서 이관.
    #   법정 부담금이 아니라 공급자 약관·계약 비용이다(전기사업법에 「시설부담금」 조문 0회 —
    #   §51 부담금은 전력산업기반부담금으로 전기 사용자 부과, 별개). 이 파일의 note 가
    #   이미 *"한전 시설분담금"*·*"도시가스 공급규정"* 이라 적고 있었다 — 라벨만 「부담금」이었다.
    ("공사비", "전기 인입공사비", "한전 전기공급약관 시설분담금(코드 U01·개산)"),
    ("공사비", "도시가스 인입공사비", "도시가스 공급규정 설치비용 분담금(코드 U02·개산)"),
    ("공사비", "통신 인입공사비", "통신사업자 인입공사 계약(코드 U03·개산)"),
    # 일반관리비
