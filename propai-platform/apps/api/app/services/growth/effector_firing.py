...
| 효과기 | 실제 | 0건의 뜻 |
|---|---|---|
| `feature_toggle` | **살아 있는 경로** — 조건 미충족 | ★**데이터 조건**이다(배선 결함 아님). ★★**막는 축이 하나가 아니다**: 게이트는 `status='open'` **AND** `created_at >= now-6h` **AND** `down_pct>=40` **AND** `ftotal+vtotal>=10` 네 술어의 곱이다. 라이브 실측 (2026-09-13 · `GET /growth/insights?insight_type=quality_drop` · total=4=len(items) **비절단**): 표본 축 **4/4 통과** · `status='open'` **2/4** · **창 축 0/4**(행 나이 461~479시간 ≈ 19~20일) · `down_pct>=40` **0/4**. ⇒ ***`down_pct` 를 100 으로 만들어도 창 축에서 0행이다.*** 그리고 더 상위 사실은 **`quality_drop` 이 약 19일째 생산되지 않았다**는 것이다 — ★단 **고유 현상이 아니다**(R3 MEDIUM-D): 같은 조회에서 `latency_baseline` **최신 0.0h** · `latency_regression` **5.6h** 로 배치는 **지금 돌고 있고**, 비-latency 생산자 4종(`fallback_rate` 197.6h · `error_cluster` 204.0h · `recurring_verify_error` 204.3h · `quality_drop` 461.6h)이 **함께 조용하다**. ⇒ `_analyze_quality_drop` 을 파기 전에 **공통 원인(입력 이벤트 고갈 등)을 먼저 보라.** ★그 공통 원인 자체는 **미측정**이다(단정하지 않는다). 잠금: `tests/test_feature_toggle_band_is_reachable.py` |
