...
# ── 갭법규 보강(P2) 법령 공식명 상수 ──
#   토지가격(공시지가·감정평가)·ESG(에너지효율등급)·운영(공동주택 행위허가·해체·임대차) 도메인.
_LAND_PRICE_DISCLOSURE = "부동산 가격공시에 관한 법률"  # 개별공시지가 결정·공시(약칭 부동산공시법)
_APPRAISAL = "감정평가 및 감정평가사에 관한 법률"      # 감정평가 기준(표준지공시지가 기준 원칙)
_BF_ACT = "장애인·노인·임산부 등의 편의증진 보장에 관한 법률"  # 장애물 없는 생활환경(BF) 인증
_CONDO_MGMT = "공동주택관리법"                        # 공동주택 행위허가·신고
...
LEGAL_REFERENCES: dict[str, dict[str, str]] = {
    ...
    # ── 갭법규 P2: 토지가격 — 공시지가·감정평가(land_price 적정가 산정 법령 근거) ──
    #   조문은 law.go.kr/법제처로 확인된 핵심 조항만 딥링크(제N조). 모두 verified.
    "official_land_price":        _ref(_LAND_PRICE_DISCLOSURE, "제10조", "개별공시지가의 결정·공시 등"),
    "land_appraisal":             _ref(_APPRAISAL, "제3조", "감정평가의 기준(표준지공시지가 기준 원칙)"),
    # ── 갭법규 P2: 설계/ESG — 편의증진(BF 인증)·건축물 에너지효율등급(BEEC) ──
    ...
    # ── 갭법규 P2: 운영 — 공동주택 행위허가·건축물 해체허가·임대차 보호 ──
    "condo_management":           _ref(_CONDO_MGMT, "제35조", "공동주택 행위허가 기준 등(증축·대수선·용도변경 등)"),
