...
class EnergyService:
    ...
    def calculate_peak_demand(
        self, annual_kwh: float, peak_factor: float = 2.5, building_type: str = "apartment"
    ) -> dict:
        if peak_factor == 2.5:
        ...
        return {
            ...
            "peak_demand_kw": round(avg * peak_factor, 2),
    ...
    def recommend_improvements(self, current_energy: dict, building_type: str = "apartment") -> list[dict]:
        ...
        if kwh_sqm > 200:
            recommendations.append(
                {"measure": "건물 외피 단열 강화 (외단열+고성능 창호)", "saving_pct": 25,
                 "cost_grade": "high", "priority": 1}
            )
        if current_energy.get("breakdown", {}).get("lighting", 0) > 0.2:
            recommendations.append(
                {"measure": "LED 조명 교체 + 조명 제어 시스템", "saving_pct": 15, "cost_grade": "low", "priority": 2}
            )
        if current_energy.get("breakdown", {}).get("cooling", 0) > 0.25:
            recommendations.append(
                {"measure": "고효율 공조 시스템 (VRF/지열)", "saving_pct": 20, "cost_grade": "medium", "priority": 3}
            )
        if kwh_sqm > 150:
            recommendations.append(
                {"measure": "태양광 패널 설치 (지붕면적 활용)", "saving_pct": 10, "cost_grade": "medium", "priority": 4}
            )
        if building_type in ("office", "commercial"):
            recommendations.append(
                {"measure": "BEMS(건물에너지관리시스템) 도입", "saving_pct": 12, "cost_grade": "medium", "priority": 5}
            )
