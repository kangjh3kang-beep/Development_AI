...
## 그라운딩 규칙(위반 금지)
법정 건폐율/용적률 상한은 **기본값**이다. 단 **기부채납(공공기여)·친환경(녹색건축/제로에너지/
신재생)·역세권 시프트/청년주택·공공임대·지구단위계획(상한용적률)** 등 합법적 인센티브로 **법정
상한을 초과해 상향받을 수 있음**을 반드시 함께 설명하라. 다만 **구체 상향 수치는 페이로드에
근거(완화율·지구단위계획·조례·인증)가 있을 때만 제시하고 근거를 명시**하라. 근거가 없으면
'완화 적용 시 상향 가능성(별도 검토 필요)'로 가능성은 안내하되 특정 수치를 단정하지 말라.
출처 없는 수치 단정 금지. 페이로드(analysis_json)에 없는 수치를 지어내지 말라.
...
class SiteAnalysisInterpreter(BaseInterpreter):
    ...
    def _legal_limits_block(zone_type: str) -> str:
        ...
        return (
            ...
            "- 위 상한은 기본값이며, 기부채납·친환경·역세권·공공임대·지구단위계획(상한용적률) 등 "
            "합법적 인센티브로 상향 가능. 단 구체 상향 수치는 페이로드에 근거(완화율/지구단위계획/"
            "조례/인증)가 있을 때만 근거와 함께 제시하고, 없으면 '상향 가능성(별도 검토)'로만 안내."
        )
