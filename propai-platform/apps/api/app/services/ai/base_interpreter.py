...
[금지 — 자주 나오는 오류(반드시 회피)]
- ★벤치마크·평균 시세·평균 분양가 등 "비교/평균 수치"를 데이터에 없으면 절대 만들어내지 않는다.
  (예: 데이터에 분양가가 없는데 "지역 평균 분양가 1,500만원/평" 같은 문장 금지 → "분양가 데이터 없음"으로만 기술)
- ★"공시지가는 시장가의 70~80% 수준" 같은 일반론적 비율·업계 관행을, 데이터 근거 없이 확정적으로 단정하지 않는다.
  (꼭 언급해야 하면 "일반적으로 알려진 경향이며 본 부지 실제 비율은 데이터 없음"처럼 불확실성을 명시)
- ★단위환산은 정확히 한다. 1평 = 3.3058㎡.
    · 단가(원/평) = 단가(원/㎡) × 3.3058  ← ㎡당 단가에 3.3058을 곱한다(절대 나누지 않는다).
    · 면적(평) = 면적(㎡) ÷ 3.3058
  계산 결과를 적을 땐 사용한 식과 입력값을 함께 적어 검산 가능하게 한다.
  (예: 1,419,000원/㎡ × 3.3058 ≈ 469만원/평)

[근거 표기 계약 — 결론성 수치에 근거를 붙인다 (v3 추가)]
- 판단·결론의 핵심 수치에는 근거를 함께 밝힌다:
  값 / 산정근거(계산식·입력값) / 출처(어느 분석 단계·서비스) / 신뢰도(높음·보통·낮음).
- 용적률·건폐율 등 법규 한도는 '법정 상한'과 '조례 실효치'를 반드시 구분해 적고, 실효치를 법정치로 낙관 대체하지 않는다.
- 신뢰도가 '낮음'(추정·미검증)인 값은 그 사실을 숨기지 말고 분명히 표기한다(옅게 흐리지 말 것).
- 법령·조례를 인용할 땐 확인된(verified) 정본 근거만 링크·조문으로 제시한다."""

# 프롬프트/그라운딩 규칙 버전 — 바뀌면 캐시키에 반영돼 기존 해석 캐시가 무효화된다.
...
class BaseInterpreter:
    ...
    def _regional_benchmark(address: str = "", region: str = "") -> str | None:
        ...
        try:
            ...
            if man_won:
                ...
                # ★이 값은 '참고용 추정 시세 테이블'이지 본 부지의 수집 데이터가 아니다.
                #  LLM이 이를 '지역 평균 분양가' 같은 확정 사실로 단정하면 검증에서 오류로 적발된다.
                #  → 내부 적정성 판단용으로만 쓰고, 언급 시 반드시 '참고 추정(원본 데이터 아님)'임을 명시.
                return (
                    f"[참고용 추정 시세(원본 수집 데이터 아님)] 이 지역 일반 분양가는 "
                    f"대략 {man_won:,}만원/평(약 {won_per_sqm:,}원/㎡) 수준일 수 있음 — "
                    f"2026년 보수적 추정 테이블 기준. "
                    f"분석 데이터 가격의 적정성 '내부 판단'에만 참고하라. "
                    f"이 수치를 '지역 평균 분양가' 등 확정 사실로 단정하지 말고, "
                    f"부득이 언급할 땐 반드시 '참고 추정(원본 데이터 아님)'임을 함께 명시하라."
                )
        except Exception:  # noqa: BLE001
            pass
        return None
    ...
    async def _invoke(
        ...
        *,
        ...
        result, parse_ok = self._parse_response_ex(raw_text)
        ...
        # P4: 결과를 L1·L2 모두에 저장 — ★성공 파싱만. 파싱 폴백({fallback_key: 원문})은
        # truthy라 기존엔 성공과 구분 없이 캐시에 박제됐다(#411 라우터 폴백 캐시오염과
        # 동일 클래스·registry '성공만 캐시' 선례의 기반클래스 일반화). 폴백은 미저장 →
        # 다음 호출이 재시도해 자가치유된다.
        if cache_key and result and parse_ok:
    ...
    def _parse_response(self, raw: str) -> dict[str, str]:
        return self._parse_response_ex(raw)[0]

    def _parse_response_ex(self, raw: str) -> tuple[dict[str, str], bool]:
        """공통 파서 + 파싱 성공 여부.

        두 번째 값(parse_ok)이 False면 반환 dict는 강등 폴백(원문 절삭) — _invoke가
        이를 L1/L2 캐시에 박제하지 않기 위한 내부 계약(#411 폴백 캐시오염과 동일
        클래스: 강등 결과가 truthy라 성공과 구분 없이 캐시되던 결함의 봉합점).
        """
        ...
        try:
            ...
            return ({self.fallback_key: extract_json_text(raw)[:500]} if self.fallback_key else {}), False
