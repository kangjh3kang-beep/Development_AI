...
_RULES = (
    ...
    DecisionRule(
        rule_id="legal.rights_takeover",
        condition="감정가 대비 인수권리 실효가치·인수율 정량 판정(★감정평가사 통합)",
        judgment=("실효가치=감정가−인수권리(선순위 (근)저당 잔액·대항력 임차보증금), 인수율=인수권리/감정가. "
                  "인수권리가 감정가 이상이면(실효가치 ≤ 0) 경제성 없음 — 매입 부적격(BLOCK)"),
        basis="민사집행법(말소기준·배당)·주택임대차보호법(대항력)·감정평가에 관한 규칙(감정가)",
        tradeoff="감정가 보수 채택(실효가치↓·안전) / 인수권리 적극 소멸 가정(실효가치↑·인수위험)",
        exception="감정가는 정식 감정 전 탁상 추정일 수 있음 — 인수권리 누락 시 실효가치 과대(보수적 가정)",
        reasoning_blueprint="감정가 확보→인수권리(선순위·대항보증금) 합산→실효가치·인수율 산정→경제성 게이트",
    ),
    DecisionRule(
