...
async def link_membership(db: AsyncSession, site_id, applicant_user_id, approver_node,
    ...
    ## ★교차 테넌트를 **의도적으로 허용한다** (2026-09-09 · R3 리뷰 M-4 에 대한 답)

    형제 생산자 `org/service.assign_user_to_node` 는 *"교차테넌트 배정은 차단(같은
    organization 사용자만)"* 을 **명문으로 강제**한다. 이 문은 **안 본다** — 규칙이 정반대다.
    **그것이 설계다.** 결정 기록(2026-09-08 회원체계 통합설계)이 그 제약을 **두 명제로 가른다**:

    | 명제 | 판단 |
    |---|---|
    | **「1인 1소속」** — 업계 표준 + 공인중개사법 §12②(분양대행 적용 여부는 **미확인**) | **지킨다** ⇒ `assign_user_to_node` 의 차단은 **그대로 둔다** |
    | **「외부인을 들일 통로가 없다」** — 병목이고 표준 아님 | **고친다** ⇒ 이 문이 그 **통로**다 |

    ★두 문은 **다른 일**을 한다: `assign_user_to_node` 는 **관리자가 기존 노드에 사람을 꽂는**
      것(내부 배정)이고, 이 문은 **본인이 신청하고 승인받아 새 노드가 생기는** 것(외부 유입)이다.
      전자의 테넌트 차단을 *"두 문이 다르니 통일하자"* 며 지우면 **그것이 회귀다** —
      `tests/test_site_join_pipeline.py::test_sibling_tenant_guard_is_not_removed` 가 막는다.

    ★**미측정**: 공인중개사법 §12② 의 분양대행 적용 여부는 확인하지 못했다(결정 기록도 그렇게
      적어 뒀다). 그 판단이 뒤집히면 「1인 1소속」의 근거가 바뀐다 — 규칙이 아니라 **근거**가.
