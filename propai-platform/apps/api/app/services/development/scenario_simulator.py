...
_SCHEME_LEGAL_KEYS: dict[str, list[str]] = {
    ...
    "결합건축": ["bldg_far"],
    "도시혁신구역": ["zone_use"],   # ★국토계획법 §40의3(구 §40의2 입지규제최소구역 — 2024.2.6 삭제)
...
#: ★폐지 제도명을 코드에 흩뿌리지 않는다 — **한 곳에서 만든다.**
#:   `입지규제최소구역`(국토계획법 §40의2)은 **2024.2.6 삭제**됐고 **도시혁신구역**(§40의3)이
#:   승계했다(법제처 DRF 원문 직접 확인 · 2026-09-05). 이름이 여러 곳에 있으면 한 곳만 고쳐진다.
SPECIAL_ZONE_SCHEME = "도시혁신구역"
#: ★옛 이름으로 검색·문의하는 사용자가 길을 잃지 않게 **승계 사실을 함께** 말한다.
#:   폐지 제도를 그냥 지우면 *"전에 되던 것이 왜 없어졌나"* 가 되고, 그냥 두면 **없는 제도를 권한다.**
SPECIAL_ZONE_SUCCESSION_NOTE = (
    "※ 구 「입지규제최소구역」(국토계획법 §40의2)은 2024.2.6 삭제되고 "
    "「도시혁신구역」(§40의3)으로 승계됐습니다."
)
    ...
    def _scenarios(self, c: dict) -> list[dict]:
        ...
        if station and seoul:
            ...
            add("역세권 활성화사업", "불가", None, None,
                ...
                f"대체: 지구단위계획(역세권 용적 완화)·{SPECIAL_ZONE_SCHEME}·도심복합개발사업 "
        ...
        # 17) ★도시혁신구역 (국토계획법 §40의3) — **입지규제최소구역(§40의2)의 승계 제도**
        #
        #   ★★**폐지된 제도를 「추진 가능」으로 권하고 있었다.** `#940` 이 이 시나리오를
        #     「불가 → 조건부」로 **승격**시켰는데(당시 게이트 축이 틀렸으므로 승격 자체는 옳았다),
        #     **제도 자체가 그 사이 폐지됐다.** 승격만 하고 라벨을 안 고친 조합이다.
        #
        ...
        #     ★기록(볼트 문서)을 승계하지 않고 **원문으로 다시 확인**했다 —
        #       법령은 기록이 가장 빨리 낡는 축이다.
        #
        #   ★**시나리오를 지우지 않는다** — 제도는 **승계**됐지 없어진 것이 아니다.
        #   ★**판정 조건은 그대로 둔다** — 도시혁신구역의 지정 요건이 입지규제최소구역과
        #     같은지는 **미확인**이다(§40의3 전문·시행령 미독). **지어내지 않는다.**
        if area >= 5000 and (station or com):
            add(SPECIAL_ZONE_SCHEME, "조건부", (far or 0) * 1.5, 30,
                ["도시지역 내 거점(역세권·복합환승 등)", f"{SPECIAL_ZONE_SCHEME} 지정", "건축·도시 융복합 계획"],
                ["용도·밀도·높이 제약 최소화", "복합 고밀개발"],
                ["구역지정·계획 심의", "공공기여"],
                "도심 거점 융복합 — 용도·밀도 제약 최소화(지정 필요). " + SPECIAL_ZONE_SUCCESSION_NOTE)
        else:
            add(SPECIAL_ZONE_SCHEME, "불가", None, None,
                ["도심 거점·5천㎡↑·지정 필요"], [],
