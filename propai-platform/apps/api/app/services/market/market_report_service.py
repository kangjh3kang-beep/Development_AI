...
def _extract_region_keys(address: str | None) -> tuple[str | None, str | None]:
    ...
    si = next(
        (t for t in reversed(toks) if t.endswith("시") and not t.endswith(("광역시", "특별시", "특별자치시"))), None
    )
...
class MarketReportService:
    ...
    async def _category_stats(self, lawd_cd: str) -> dict[str, Any]:
        ...
        async def trade_one(pt: str, label: str):
            ...
            res = await asyncio.gather(
                *[self.molit.get_transactions(lawd_cd, ym, prop_type=pt, num_rows=1000) for ym in months],
            ...
            return label, {
                **_stat(prices),
                "avg_area_m2": round(sum(a for a in areas if a > 0) / max(1, len([a for a in areas if a > 0])), 1)
                if areas
                else 0,
        ...
        async def rent_one(pt: str, label: str):
            rows: list = []
            res = await asyncio.gather(
                *[self.molit.get_rent_transactions(lawd_cd, ym, prop_type=pt, num_rows=1000) for ym in months],
                return_exceptions=True,
            )
            for r in res:
                if isinstance(r, list):
    ...
    async def _narrative(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            sys = (
                "당신은 부동산 개발 및 시장분석 전문가다. "
                "제공된 실거래·시세·입지 데이터와 인구 이동, 연령대, 평균 소득 데이터를 종합하여 "
                ...
                "★target_persona에는 유입 인구의 주 연령대, 거시적 평균 소득을 고려해 "
                "가장 분양 가능성이 높은 고객의 직업군/가구형태/특화설계 제안을 포함하라."
            )
            ...
            return {
                "summary": "수집된 실거래·시세 데이터를 기반으로 한 시장 현황입니다.",
                "opportunities": [],
                "risks": [],
                "price_trend": "",
                "target_persona": "데이터 기반 타겟팅 분석 불가",
            }
    ...
    async def _nearby_presale_84_price(
        ...
        try:
            ...
            near = await _aio.wait_for(
                svc.nearby(_lat, _lon, area_from_lawd(lawd_cd), radius_m=3000, months_back=12, max_markers=8),
                timeout=12.0,
            )
    ...
    async def build_report(
        ...
        if want_pop or want_mig or want_inc:
            try:
                ...
                async def fetch_pop():
                    if not want_pop:
                        return {"target_adm_cd": lawd_cd, "year": cur_year}
                    return await sgis.get_population_stats(lawd_cd, cur_year, region_name=sgis_nm)

                async def fetch_inc():
                    if not want_inc:
                        return {"sigungu_cd": lawd_cd[:5], "year": cur_year}
                    return await kosis.get_macro_income_stats(lawd_cd[:5], cur_year, region_name=kosis_nm)

                mig, pop, inc = await asyncio.gather(fetch_mig(), fetch_pop(), fetch_inc(), return_exceptions=True)
                # Pydantic 모델을 사용해 어댑터 패턴으로 데이터 표준화
                profile = DemographicProfile(
                    source_phase=1,
                    migration=MigrationData(
                        **(mig if not isinstance(mig, Exception) else {"target_adm_cd": lawd_cd, "year": cur_year})
                    ),
                    population=PopulationData(
                        **(pop if not isinstance(pop, Exception) else {"target_adm_cd": lawd_cd, "year": cur_year})
                    ),
                    macro_income=MacroIncomeData(
                        **(inc if not isinstance(inc, Exception) else {"sigungu_cd": lawd_cd[:5], "year": cur_year})
                    ),
                )
        ...
        zone_type = (
            zone.get("zone_type") or land_use.get("zone_type") or basic.get("zone_type") or comp.get("zone_type")
        )
        official_price = None
        ...
        ctx = {
            ...
            "apt_trend_per_pyeong": [
                {"ym": t["ym"], "per_pyeong_manwon": t.get("avg_per_pyeong")} for t in (stats.get("apt_trend") or [])
            ],
    ...
    @staticmethod
    def static_map_png(
        lat: float, lon: float, radius_m: int = 1000, zoom: int = 15, w: int = 720, h: int = 440
    ) -> bytes | None:
    ...
    def to_pdf(self, rep: dict[str, Any]) -> bytes:
        ...
        h2 = ParagraphStyle(
            "h2", parent=ss["Heading2"], fontName=font, fontSize=13, textColor=colors.HexColor("#0e7490")
        )
        ...
        if _intg.get("parcel_count") and (_intg.get("total_area_sqm") or 0) > 0:
            ...
            story.append(
                Paragraph(
                    ...
                    f"{_ta:,.0f}㎡({round(_ta / PYEONG_SQM):,}평) 기준 분석",
                    body,
                )
            )
        ...
        if rep.get("zone_type") or rep.get("official_price_per_sqm"):
            ...
            story.append(Paragraph(f"용도지역: {_esc(rep.get('zone_type') or '-')} · 공시지가(㎡): {opp_txt}", body))
        ...
        def stat_table(title: str, data: dict, unit_label: str):
            ...
            for label, s in data.items():
                rows.append(
                    [label, str(s.get("count", 0)), _eok(s.get("avg", 0)), _eok(s.get("min", 0)), _eok(s.get("max", 0))]
                )
        ...
        if _fin and not fe.get("error"):
            ...
            if mix and um.get("data_source") not in (None, "unavailable"):
                ...
                story.append(Paragraph("권장 평형 배분: 데이터 없음(인구/가구 분석 미선택 — 가구원수 분포 필요)", cap))
            story.append(Spacer(1, 8))
        ...
        if pop and not _is_real_source(pop.get("data_source")):
            ...
            srows = [
                ...
                [
                    "평균 가구원수",
                    f"{summ.get('avg_household_size')}명" if summ.get("avg_household_size") else "데이터 없음",
                ],
            ]
            ...
            if age:
                arows = [["연령대", "인구수"]] + [
                    [a.get("label", "-"), f"{a.get('count'):,}" if a.get("count") else "-"] for a in age
                ]
            ...
            if ht:
                hrows = [["가구원수", "비율(%)"]] + [
                    [h.get("label", "-"), f"{h.get('ratio')}%(추정)" if h.get("ratio") is not None else "-"] for h in ht
                ]
        ...
        if pb and pb.get("data_source") not in (None, "unavailable") and pb.get("fair_price_10k"):
            ...
            story.append(
                Paragraph(
                    ...
                    f"지불여력 판정: {_esc(pb.get('affordability_verdict', '-'))}",
                    body,
                )
            )
        ...
        story.append(Paragraph(_esc(_ptrend) if _ptrend else _ai_missing, body if _ptrend else cap))
    ...
    def to_pptx(self, rep: dict[str, Any]) -> bytes:
        ...
        def title_slide():
            ...
            brand.paragraphs[0].font.size = Pt(16)
            brand.paragraphs[0].font.color.rgb = accent
            brand.paragraphs[0].font.bold = True
            tb = s.shapes.add_textbox(Inches(0.9), Inches(2.5), Inches(11.7), Inches(2.5)).text_frame
            tb.text = "시장조사보고서"
            tb.paragraphs[0].font.size = Pt(52)
            ...
            disc.paragraphs[0].font.size = Pt(9)
            disc.paragraphs[0].font.color.rgb = RGBColor(0x94, 0xA3, 0xB8)
        ...
        def kv_table_slide(title: str, header: list[str], data_rows: list[list[str]], caption: str | None = None):
            ...
            tbl = s.shapes.add_table(
                rows, ncol, Inches(0.8), Inches(1.5), Inches(11.7), Inches(min(0.5 * rows, 5))
            ).table
        ...
        if pop:
            ...
            for h in pop.get("household_types") or []:
                prows.append(
                    [f"가구원 {h.get('label', '-')}", f"{h.get('ratio')}%(추정)" if h.get("ratio") is not None else "-"]
                )
            mig = pop.get("migration") or {}
            prows.append(
                [
    ...
    def to_docx(self, rep: dict[str, Any]) -> bytes:
        ...
        def _table(header: list[str], data_rows: list[list[str]]):
            ...
            with contextlib.suppress(Exception):  # 스타일 없으면 기본 스타일 유지
        ...
        if pop:
            ...
            _table(
                ...
                [
                    [
                        "총인구",
                        f"{summ.get('total_population'):,}명" if summ.get("total_population") else "데이터 없음",
                    ],
                    [
                        "가구수",
                        f"{summ.get('household_count'):,}가구" if summ.get("household_count") else "데이터 없음",
                    ],
                    [
                        "평균 가구원수",
                        f"{summ.get('avg_household_size')}명" if summ.get("avg_household_size") else "데이터 없음",
            ...
            if age:
                ...
                _table(
                    ["연령대", "인구수"],
                    [[a.get("label", "-"), f"{a.get('count'):,}명" if a.get("count") else "-"] for a in age],
                )
            ht = pop.get("household_types") or []
            if ht:
                ...
                _table(
                    ["가구원수", "비율(%)"],
                    [
                        [h.get("label", "-"), f"{h.get('ratio')}%(추정)" if h.get("ratio") is not None else "-"]
                        for h in ht
                    ],
                )
            mig = pop.get("migration") or {}
        ...
        if pb and pb.get("data_source") not in (None, "unavailable") and pb.get("fair_price_10k"):
            ...
            doc.add_paragraph(
                pb.get("note") if pb.get("note") else "적정 분양가 산출 불가(비교 데이터 부족) — 가짜값 미생성"
            )
