...
async def _broadcast(site_id, event: str, unit_id, status: str, held_by=None, expires_at=None):
    ...
    ★[security·개인정보 — #2 backlog 정합] held_by(점유 직원 UUID)는 broadcast 에 절대 싣지 않는다.
      WS broadcast 는 채널의 모든 구독자에게 '동일한 한 페이로드'를 그대로 뿌리므로(수신자별 권한
      구분 불가), held_by 평문을 실으면 GET /units/board 의 마스킹(_mask_held_by: 본인/관리자만 노출)을
      우회해 일반 멤버에게도 점유자 신원이 새어나간다. 보드 표시에는 점유여부(status=HOLD)만 필요하고
      '내가 잡았는지'는 클라이언트가 자신의 hold_token 보유로 판단하므로(held_by_me), held_by 평문 push 는
      불필요·유해하다. 점유자 신원이 필요한 화면은 권한 마스킹이 적용된 GET 보드를 재조회한다.
      (held_by 인자는 호출부 시그니처 호환을 위해 남기되 페이로드에서 의도적으로 제외한다.)
    """
    payload = {
        "type": "UNIT_STATUS",
