...
class DesignParams(BaseModel):
    ...
    floors: int | None = Field(None, ge=0, description="지상 층수")
    floor_height_m: float | None = Field(None, gt=0, description="층고(m), 기본 3.0")
    gfa: float | None = Field(None, gt=0, description="연면적(㎡)")
    bcr: float | None = Field(None, ge=0, description="계획 건폐율(%)")
    far: float | None = Field(None, ge=0, description="계획 용적률(%)")
    height_m: float | None = Field(None, ge=0, description="건물 높이(m)")
    parking: int | None = Field(None, ge=0, description="계획 주차대수")
    units: int | None = Field(None, ge=0, description="세대/호수")
    building_type: str | None = Field(None, description="건물 용도(아파트·근린생활시설 등)")
    avg_unit_area_sqm: float | None = Field(None, gt=0, description="평균 전용면적(㎡)")
    land_area_sqm: float | None = Field(None, gt=0, description="대지면적(㎡)")
    building_area_sqm: float | None = Field(None, gt=0, description="건축면적(㎡)")
...
async def _augment_from_site(
    address: str | None, pnu: str | None, zone_type: str | None
) -> dict[str, Any]:
