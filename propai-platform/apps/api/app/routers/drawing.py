...
class AutoDesignRequest(BaseModel):
    """AI 자동 설계 생성 요청."""
    site_area_sqm: float = Field(..., gt=0, description="대지면적 (sqm)")
    site_shape: Optional[list[dict[str, float]]] = Field(None, description="대지 형상 좌표")
    site_width_m: float = Field(0.0, ge=0, description="대지 폭 (m)")
    site_depth_m: float = Field(0.0, ge=0, description="대지 깊이 (m)")
...
class CalculateAreaRequest(BaseModel):
    ...
    site_area_sqm: float = Field(0.0, ge=0, description="대지면적 (m²)")
...
async def generate_site_plan(req: SitePlanRequest):
    svg = svg_service.generate_site_plan(req.site_width_m, req.site_depth_m,
...
async def generate_floor_plan(req: FloorPlanRequest):
    svg = svg_service.generate_floor_plan(req.total_floor_area_sqm, req.unit_type,
...
async def export_dxf(req: ExportDxfRequest):
    ...
    drawing_type: floor_plan | detailed | section | elevation_front | elevation_side | site_plan
    """
    dt = req.drawing_type
    filename = f"{dt}.dxf"
...
async def calculate_area(req: CalculateAreaRequest):
    """DesignPayload에서 폴리곤별 면적 + 건폐율/용적률을 계산한다."""
