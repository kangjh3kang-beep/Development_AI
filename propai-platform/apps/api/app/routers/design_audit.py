...
async def _execute_run(
    ...
    # ★로드맵③ — deliberation_surface_in_audit(별도 게이트, 기본 False)가 추가로 켜졌을 때만 이 관측을
    #   fire-and-forget 대신 **대기**(bounded — shadow_compare 자체 timeout=deliberation_shadow_engine_timeout_s)
    #   해 결과를 응답에 additive 동봉한다(deliberation_result). 두 게이트 모두 기본 off이므로 기본 응답은
    #   기존과 바이트까지 동일(무변경) — 운영이 플래그를 켜면 표면화, 코드가 운영 결정을 선점하지 않는다.
    deliberation_result: dict[str, Any] | None = None
    try:
        from apps.api.config import get_settings

        _settings = get_settings()
        if _settings.deliberation_shadow_enabled:  # gate-first(off면 매퍼/스케줄 미발생) — 기존 동작 보존
            ...
            if getattr(_settings, "deliberation_surface_in_audit", False) and _mapped and _tid:
                ...
                shadow_integration.observe(  # 표면화 off — 기존 비차단 관측(엔진 RTT가 심사 응답을 막지 않음)
                    "design_audit", _tid, _mapped)
...
_AUDIT_JOB_TTL = 3600.0


def _prune_audit_jobs() -> None:
    now = time.time()
    for k in [k for k, v in _AUDIT_JOBS.items() if now - v.get("ts", 0) > _AUDIT_JOB_TTL]:
        _AUDIT_JOBS.pop(k, None)


def _audit_job_set(job_id: str, **fields: Any) -> None:
    """job_id 항목을 병합 갱신(user_id 등 기존 필드 보존) + ts(TTL 기준) 갱신."""
    cur = dict(_AUDIT_JOBS.get(job_id) or {})
    cur.update(fields)
    cur["ts"] = time.time()
    _AUDIT_JOBS[job_id] = cur
...
async def _run_audit_upload_job(
    ...
    """
    _audit_job_set(job_id, status="running")
    try:
        ...
        _audit_job_set(job_id, status="done", result=resp)
        ...
        _audit_job_set(job_id, status="error", error=str(e)[:200])
