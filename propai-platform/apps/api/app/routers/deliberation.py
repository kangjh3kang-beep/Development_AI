...
def _compat_fields(result: dict[str, Any] | None) -> dict[str, Any]:
    """HEAD `_wrap_result` 호환 평면 필드(findings·complianceScore·finalStatus·sections)를 산출한다.

    ★union 보존: main 단독 소비처(apps/web/.../DeliberationResultPanel.tsx, node-registry audit 노드)가
    봉투 최상위의 `complianceScore`/`finalStatus`/`findings`를 읽으므로, 브랜치 봉투(result 중첩)에
    이 평면 필드를 함께 노출해 양측 계약을 모두 만족시킨다(가공 실패가 전체를 깨지 않게 best-effort).
    결정론 산출 수치는 변형하지 않는다(표면화 전용).
    """
    if not isinstance(result, dict):
        return {"complianceScore": None, "finalStatus": None, "findings": [], "sections": {}}
