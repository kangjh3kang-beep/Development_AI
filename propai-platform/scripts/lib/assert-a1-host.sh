...
assert_a1_host() {
  ...
  # ★★R4(독립 리뷰 MEDIUM-3) — **「모름」이 「확신」으로 나오고 있었다.**
  #   `[ -z "$self_repo" ]` 만 보면 **측정 실패가 값처럼 통과**한다. 실측:
  #       source /dev/stdin < 이 파일  →  self_repo="/"   ← 「/」 는 값이 아니라 **해석 실패**다
  #       그런데 비어 있지 않으므로 `wrong-checkout`(=「다른 체크아웃에서 돌고 있다」)로 **단정**했다.
  #   ⇒ ***이 파일의 결함이 다섯 번째 얼굴로 돌아온 것이다***:
  #       ①실패 문구 ②종료코드 ③부재→A1 유도 ④상태명 ⑤**해석 실패→「다르다」 유도**
  #   ★그리고 형제도 죽어 있었다(MEDIUM-2): `cd …/../../.. && pwd -P` 는 **거의 언제나 성공**해
  #     `unknown-checkout` 이 사실상 도달 불가였다. 셋 중 둘이 죽은 채 하나만 표시돼 있었다.
  #
  # ★처방 — **값이 있는지**가 아니라 **저장소 루트처럼 생겼는지**를 본다(한 줄이 셋을 정리한다).
  #   이러면 `unknown-checkout` 이 **실제로 도달 가능**해지고, 위 재현이 **정직한 「모름」**이 된다.
  local kind
  if [ -z "$self_repo" ] || [ ! -d "$self_repo/propai-platform/scripts" ]; then
    kind="unknown-checkout"      # 내가 어디서 도는지 **못 쟀다** — 단정하지 않는다
  elif [ "$self_repo" != "$repo" ]; then
    kind="wrong-checkout"        # 기대한 저장소와 **다른 체크아웃**에서 돌고 있다
  else
    kind="repo-missing"          # 두 값이 같은데 디렉토리 검사가 실패했다(원인 미상 — 아래)
  fi
...
★판정: ${kind}
  ...
  · unknown-checkout → **내가 어디서 도는지 못 쟀습니다.** 아래 「지금 실행 중인 저장소」를
      믿지 마십시오(해석 실패일 수 있습니다). 이 경우에도 **상수는 고치지 마십시오.**
  · repo-missing     → 두 값이 **같은데 디렉토리 검사가 실패**했습니다.
    ★**원인을 단정하지 않습니다** — 후보: 권한(EACCES) · 심링크 · 경합(삭제와 실행이 겹침) ·
      원격FS stale. (동료 실측으로 «\$REPO 가 디렉토리가 아님» 후보는 **지워졌습니다** —
      그 경우 물리 경로가 달라져 wrong-checkout 이 납니다.)
    ***진단을 못 하는 자리에서 진단을 적으면 그게 다음 사람을 틀린 곳으로 보냅니다*** —
    이 파일이 처음부터 말하던 그것입니다.
WRONGHOST
  exit 11
}
