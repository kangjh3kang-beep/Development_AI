...
- 위치: 우리 저장소의 공유 git 디렉토리 `.git/coordination/BOARD.md` (= `git rev-parse --git-common-dir`/coordination). **모든 워크트리가 같은 한 부를 보며, 정확히 이 저장소에만 스코프**된다(상위 공용 디렉토리 아님). git 미추적이라 브랜치무관·머지충돌 없음. 조회: `scripts/coord.sh status`.
